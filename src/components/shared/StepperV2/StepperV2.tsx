...
const StepperV2 = ({
  ...
  // const isLpaReview = module === 'LPA' && (process === 'LPA' || process === 'LPA_REVIEW');
  // const refetchInterval = isLpaReview ? false : 5000;
  ...
  function handleClickStepper(lastPath: string, viewOnly: boolean) {
    ...
    const basePath: string = `${segments.slice(0, -1).join('/')}[processId]/${lastPath}`;
        ...
        >
          {
            steps.map((step, index) => {
              ...
              return (
                <Box
                  key={index}
                  ref={matchesPathname(lastPath, step.urlPath) ? ref : null}
                  ...
                  {step.isButtonShow && (
                    <Box
                      sx={{
                        ...
                        border: `0.15vw solid ${theme.palette.common.white}`,
                        borderRadius: '50%',
                        height: '1.2vw',
                        position: 'absolute',
                        right: '3.2vw',
