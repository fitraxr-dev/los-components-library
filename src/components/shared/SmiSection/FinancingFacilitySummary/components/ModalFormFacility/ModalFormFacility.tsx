...
const ModalFormFacility = NiceModal.create((
  { existing, id, module, process }: any) => {
  const theme = useTheme();
  const { facilityId } = useIdentity();
  const modalId = modalMip.FORM_FACILITY;
  ...
  } = useModalFormFacility({ existing, id });
  ...
  let title = '';
  if (existing) {
    title = 'Add Fasilitas Pembiayaan Eksisting';
  } else if (facilityId) {
    title = 'Edit Fasilitas Pembiayaan';
  } else {
    title = 'Add New Fasilitas Pembiayaan';
  }
