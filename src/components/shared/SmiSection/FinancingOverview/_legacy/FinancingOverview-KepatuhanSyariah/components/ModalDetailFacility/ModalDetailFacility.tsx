...
const ModalDetailFacility = NiceModal.create(({ id, facilityId, bucketProcessId }: any) => {
  ...
  const {
    ...
    financingSegment,
    projectData,
  } = useDetailFacility(id, facilityId, bucketProcessId);
        ...
        <ColumnWrapper>
          ...
          >
            {facilityData?.map((item, index) =>
              <Box
                key={index}
                sx={item.key === 'remark' ? { gridColumn: '1 / -1' } : {}}
              >
                <Cell title={item.label} value={item.value} />
              </Box>
            )}
          </Box>
        </ColumnWrapper>

        {financingSegment === 'SYARIAH' && financingData && financingData.length > 0 && (
          <ColumnWrapper>
            <TextStyle
              variant="body3"
              weight={600}
