...
const useAlQardh = (props: SyariahFormsProps) => {
  ...
  useMemo(() => masintonChange('debtorName', debtorDetail?.debtorName), [debtorDetail]); const { data: currencyDropdownList } = useGetParameterList(Modules.CURRENCY, { label: 'value1', rate: 'value2', value: 'key' });
  ...
  useEffect(() => {
    if (currencyDropdownList && financingFacilityData && !exchange_rate_al_qardh_loan.value) {
      ...
      if (finalRate) {
        masintonChange('exchange_rate_al_qardh_loan', finalRate);
      }
    }
  }, [currencyDropdownList, financingFacilityData, exchange_rate_al_qardh_loan.value]);
