...
import useImbt from './ImbtForm.hook';

import type { SyariahFormsProps } from '../forms.type';


const ImbtForm = (props: SyariahFormsProps) => {
  ...
  return (
    <>
      ...
      >
        <Input
          label={getLabel('debtor_name', 'Penyewa / Musta\'jir')}
          type="text"
          disabled
          placeholder={getLabel('debtor_name', 'Penyewa / Musta\'jir')}
          containerSx={{ flex: 1 }}
      ...
      <Box border="1px solid #D3D3D3" borderRadius="10px">
        ...
        >
          ...
          >
            ...
            <Input
              label={getLabel('imbt_object', 'Object IMBT')}
              placeholder={getLabel('imbt_object', 'Object IMBT')}
              containerSx={{ flex: 1 }}
              value={imbt_object.value}
              onChange={(val) => masintonChange('imbt_object', val)}
              error={imbt_object.error}
              helperText={imbt_object.error && imbt_object.errorMessage}
            />
          </Box>
          ...
          >
            <Box sx={{ position: 'relative' }}>
              <Box sx={{ alignItems: 'center', display: 'flex', gap: 1, mb: 1 }}>
                <TextStyle variant="body4" weight={600} color={theme.palette.custom.text}>
                  Nilai Pengalihan Objek IMBT
                </TextStyle>
              </Box>
              <CurrencyForm
                initialProps={{
                  currency: currency_imbt_transfer_value.value,
                  error: imbt_transfer_value.error,
                  errorMessage: imbt_transfer_value.errorMessage,
                  label: '',
                  onChange: (val) => {
                    masintonChange('imbt_transfer_value', val.value);
                  },
                  onCurrencyChange: (val) => {
                    if (val.currency === 'IDR') {
                      masintonMultiChange({
                        currency_imbt_transfer_value: val,
                        exchange_rate_imbt_transfer: '',
                        imbt_transfer_value_idr: '',
                      });
                    } else {
                      masintonMultiChange({
                        currency_imbt_transfer_value: val,
                        exchange_rate_imbt_transfer: currencyDropdownList.find((dt) => dt.value === val)?.rate,
                      });
                    }
                  },
                  placeholder: 'Nilai Pengalihan Objek IMBT',
                  value: imbt_transfer_value.value,
                }}
                kursProps={{
                  error: exchange_rate_imbt_transfer.error,
                  errorMessage: exchange_rate_imbt_transfer.errorMessage,
                  isMandatory: !showTooltips,
                  onChange: (val) =>
                    masintonChange('exchange_rate_imbt_transfer', val.value),
                  value: exchange_rate_imbt_transfer.value,
                }}
                idrProps={{
                  error: imbt_transfer_value_idr.error,
                  errorMessage: imbt_transfer_value_idr.errorMessage,
                  value: imbt_transfer_value_idr.value,
                }}
              />
            </Box>
            <Input
              label={getLabel('imbt_object_transfer_agreement', 'Akad Pengalihan Objek IMBT')}
              type="dropdown"
              placeholder={getLabel('imbt_object_transfer_agreement', 'Akad Pengalihan Objek IMBT')}
              containerSx={{ flex: 1 }}
              dropdownList={Dimbt_object_transfer_agreement}
              value={imbt_object_transfer_agreement.value}
              onChange={(val) => masintonMultiChange({
                imbt_object_transfer_agreement:
                  Dimbt_object_transfer_agreement.find((dt) => dt.value === val)?.value,
                imbt_object_transfer_agreement_label:
                  Dimbt_object_transfer_agreement.find((dt) => dt.value === val)?.label,
              })}
              error={imbt_object_transfer_agreement.error}
              helperText={imbt_object_transfer_agreement.error && imbt_object_transfer_agreement.errorMessage}
            />
          </Box>
