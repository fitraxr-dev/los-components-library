...
import { getSyariahFieldCellsSeparated } from './ModalDetailFacilitySyariah.constants';
...
const useDetailFacility = (id, idFacility?: string) => {
  const { facilityId, processId, parentId } = useIdentity();
  ...
  const { data: financingFacilityData } = useGetDetailFinancingFacility({
    ...
    facilityId: idFacility || facilityId || null,
    id: id || null,
  });
  ...
  const mapCellData = (type: 'facility' | 'project', data: any, cellData: any[]) => {
    ...
    if (type === 'facility' && isSyariahSegment) {
      const baseCells = [
        ...
        { key: 'mappingProduct', label: 'CORE Mapping Produk', value: data?.mappingProductLabel || '-' },
        { key: 'rateType', label: 'Tipe Rate', value: data?.rateTypeLabel || '-' },
        { key: 'floatingReference', label: 'Referensi Floating', value: data?.floatingReferenceLabel || '-' },
        { key: 'childFacilityAlias', label: 'Alias Fasilitas Anak', value: data?.childFacilityAlias || '-' },
    ...
    const result = cellData.map((item) => {
      ...
      if (item.label === 'CORE Mapping Segmen Pembiayaan') {
        value = data?.mappingFinancingSegmentLabel || '-';
      }

      if (item.label === 'CORE Mapping Produk') {
