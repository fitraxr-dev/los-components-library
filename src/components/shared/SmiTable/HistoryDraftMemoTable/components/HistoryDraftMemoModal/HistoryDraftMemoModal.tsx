...
import { HISTORY_DRAFT_MEMO_INITIAL_VALUES, HISTORY_DRAFT_MEMO_MODAL_SCHEMA } from './HistoryDraftMemoModal.constants';
import { useHistoryDraftMemoModal } from './HistoryDraftMemoModal.hook';


const HistoryDraftMemoModal = create(() => {
  const [state] = useApp();
  const theme = useTheme();
  const modalId = MODAL.PROPOSAL.HISTORY_DRAFT_MEMO;
  ...
  const { handleOnSave, saveHistoryDraftLoading } = useHistoryDraftMemoModal();
          ...
          <Controller
            ...
            render={({ field: { ref, ...field }, fieldState: { invalid, error, isTouched } }) => (
              <Input
                ...
                helperText={fileError || (isTouched && error ? error.message : '')
...
export default HistoryDraftMemoModal;
