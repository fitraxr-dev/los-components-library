...
export const useTableEloDocument = (props: TableUploadDocumentProps) => {
  ...
  const { processId, debtorId, debiturName } = useIdentity();
  ...
  // API call for ELO documents
  const { data: eloDocumentData, isLoading: eloDocumentLoading } = useGetDocumentEloList({
    filter: {
      ...filter?.filter,
      bucketProcessId: id !== undefined && id !== null ? String(id) : processId,
      documentCategory: DocumentTypeRequestDtoDocumentCategoryEnum.ELO,
  ...
  const eloDocumentContents = eloDocumentData?.contents;
  const eloDocumentPage = eloDocumentData?.page;
  ...
  const handleOpenAddNewModal = () => {
    NiceModal.show(MODAL_UPLOAD_DOCUMENT_ELO, { ...props, title: 'Document ELO', type: DocumentTypeRequestDtoDocumentParentEnum.ELO });
  };
  ...
  const handleOpenEditModal = (id: number) => {
    NiceModal.show(MODAL_UPLOAD_DOCUMENT_ELO, { ...props, id, title: 'Document ELO', type: DocumentTypeRequestDtoDocumentParentEnum.ELO });
  };
