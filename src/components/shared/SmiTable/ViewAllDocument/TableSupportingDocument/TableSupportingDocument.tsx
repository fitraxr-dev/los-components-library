...
import { useTableSupportingDocument } from './TableSupportingDocument.hook';

import type { TableUploadDocumentProps } from '../../TableUploadDocument/TableUploadDocument.types';
import type { TableHeader } from '@/components/shared/Table/Table.types';


const TableSupportingDocument = (props: TableUploadDocumentProps) => {
  const [state] = useApp();
  ...
  } = useTableSupportingDocument(props);
  ...
  const isDisabled = isSuperAdmin || viewOnly || readOnly;
  ...
  const handleActionButton = (row) => {
    ...
    if (!isDisabled && (row.hasSubmitted || !row.isFromOtherProcess)) {
      res.unshift({
        ...
        onClick: async (row) => handleOpenDetailModal(row.id),
      });
    }
    res.unshift({
      iconName: 'delete',
  ...
  const tableHeaderDocument: Array<TableHeader> = useSelected
    ? [
      {
      ...
      <SectionTitle title="Supporting Document" isOpen>
        ...
        <BaseContainer sx={{ boxShadow: 7 }}>
          <Table
            ...
            tableData={supportingDocumentList}
            isLoading={supportingDocumentLoading}
            currentPage={noPage}
            totalPage={supportingDocumentPage?.totalPage}
...
export default TableSupportingDocument;
