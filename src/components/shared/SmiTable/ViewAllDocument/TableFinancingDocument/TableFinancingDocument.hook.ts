...
export const useTableFinancingDocument = (props: TableUploadDocumentProps) => {
  const {
    module,
    process,
    id,
  ...
  const [filter, setFilter] = useSessionStorage(`${props.module}-${props.process}-filter-financing-document`, null);
  ...
  const dataTypes = useMemo(() => {
    const mappedData = documentTypeData?.map((item) => ({
      label: item.label,
      value: item.id,
    })) || [];

    // Hardcode KTP dan NPWP jika belum ada di response
    const hardcodedTypes = [
      { label: 'KTP', value: 'KTP' },
      { label: 'NPWP', value: 'NPWP' },
    ];

    // Gabungkan data dari API dengan hardcode, pastikan tidak duplikat
    const existingLabels = new Set(mappedData.map((item) => item.label));
    const filteredHardcoded = hardcodedTypes.filter((item) => !existingLabels.has(item.label));

    return [...mappedData, ...filteredHardcoded];
  }, [documentTypeData]);
  ...
  const handleOpenAddNewModal = () => {
    NiceModal.show(modal.MODAL_UPLOAD_DOCUMENT, { ...props, disableGroupOnKtpNpwp, title: 'Document Pembiayaan', type: DocumentTypeRequestDtoDocumentParentEnum.FINANCINGDOCUMENT });
  };
  ...
  const handleOpenEditModal = (id: number) => {
    NiceModal.show(modal.MODAL_UPLOAD_DOCUMENT, { ...props, disableGroupOnKtpNpwp, id, title: 'Document Pembiayaan', type: DocumentTypeRequestDtoDocumentParentEnum.FINANCINGDOCUMENT });
  };
