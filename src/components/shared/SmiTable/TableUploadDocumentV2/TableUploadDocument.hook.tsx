...
import { businessActivityReport, eligibilityReview, ESDD, KEPATUHAN_SYARIAH } from '@/configs/constants/pathname';
...
const useTableUploadDocument = ({
  ...
  const isBusinessActivityReport = replacePath(businessActivityReport.INFORMATION, {
    module: moduleIndex,
    processId,
  });
  ...
  const { data: documentList, isLoading: isGetDocumentListLoading } = useGetDocumentList({
    filter: {
  ...
  const renderActionsTable = () => {
    ...
    if (sequentialActionButtonsByTemplate.length > 0) {
      ...
      } else if (matchesPathname(path, isBusinessActivityReport)) {
        buttonResult = [
          ...buttonResult,
          {
            iconName: 'edit',
            isHidden: viewOnly || isReadOnly,
            onClick: (data) => handleEditDocument({ id: data?.id, module, process }),
          },
          {
            iconName: 'delete',
