        ...
        >
          ...
          >
            ...
            >
              ...
              {additionalData?.splice(0, 3).filter((item) => item.type !== 'currency').map((item, index) => {
                return (
                  <Cell key={index} title={item.label} value={item.value} />
                );
              }
            ...
            >
              <CurrencyCell
                value={financingFacilityData.imbt_transfer_value}
                currency={financingFacilityData?.currency_imbt_transfer_value}
                rate={financingFacilityData?.exchange_rate_imbt_transfer}
                idrValue={financingFacilityData?.imbt_transfer_value_idr}
                valueTitle="Nilai Pengalihan Objek IMBT"
                rateTitle="Konversi Mata Uang"
                totalTitle="Nilai Pengalihan Objek IMBT (Dalam Rp)"
              />
              <Cell title="Akad Pengalihan Objek IMBT" value={financingFacilityData.imbt_object_transfer_agreement_label} />
            </Box>
            <Box
              sx={{
                display: 'grid',
            ...
            >
              ...
              {additionalData?.splice(0, 4).filter((item) => item.type !== 'currency').map((item, index) => {
                return (
                  <Cell key={index} title={item.label} value={item.value} />
                );
              }
