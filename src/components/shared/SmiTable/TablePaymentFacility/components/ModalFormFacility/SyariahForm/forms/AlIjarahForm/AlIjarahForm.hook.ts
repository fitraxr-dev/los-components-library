...
import { AlIjarahFormData } from './AlIjarah.form';

import type { SyariahFormsProps } from '../forms.type';


const useAlIjarah = (props: SyariahFormsProps) => {
  ...
  const {
    ...
    masintonMultiChange,
    masintonMagic,
  } = useMasintonForm(AlIjarahFormData, validation);
  ...
  useEffect(() => {
    if (currencyDropdownList && financingFacilityData) {
      ...
      const isRateEmptyOrOne = !exchange_rate_facility_value.value || exchange_rate_facility_value.value === '1';

      if (isIdr) {
        if (exchange_rate_facility_value.value !== '1') {
          masintonChange('exchange_rate_facility_value', '1');
        }
      } else if (isRateEmptyOrOne) {
        const exchangeRateFromApi = financingFacilityData?.attributes?.find((attr) =>
          attr.attributeKey === 'exchange_rate_facility_value')?.attributeValue;
        ...
        if (finalRate && finalRate !== exchange_rate_facility_value.value) {
          masintonChange('exchange_rate_facility_value', finalRate);
        }
      }
    }
  }, [currencyDropdownList, financingFacilityData, exchange_rate_facility_value.value, currency_facility_value.value]);
