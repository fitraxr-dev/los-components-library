...
const AlMusyarakahMutanaqisahForm = (props: SyariahFormsProps) => {
  const { onChangeSyariahForm, financingFacilityData, syariahComponentConfig, disabled } = props;
      ...
      <Box sx={{ position: 'relative' }}>
        <Box sx={{ alignItems: 'center', display: 'flex', gap: 1, mb: 1 }}>
          <TextStyle variant="body4" weight={600} color={theme.palette.custom.text}>
            {getLabel('partnership_smi_facility', 'Syirkah SMI/Nilai Fasilitas Pembiayaan')}
          </TextStyle>
              ...
              title={
                (() => {
                  ...
                  const existingPartnershipSmiFacility = financingFacilityData?.attributes?.find((attr) => attr.attributeKey === 'existing_core_partnership_smi_facility')?.attributeValue ||
                    financingFacilityData?.attributes?.find((attr) => attr.attributeKey === 'partnership_smi_facility')?.attributeValue || '0';
                  const existingCurrencyPartnershipSmiFacility = financingFacilityData?.attributes?.find((attr) => attr.attributeKey === 'existing_core_currency_partnership_smi_facility')?.attributeValue ||
                    financingFacilityData?.attributes?.find((attr) => attr.attributeKey === 'currency_partnership_smi_facility')?.attributeValue || 'IDR';
                  ...
                  const exchangeRateFromApi = (exchange_rate_global.value && exchange_rate_global.value !== '0') ?
                    parseFloat(exchange_rate_global.value.toString().replace(/,/g, '')) :
                  ...
                  const currentCurrencySmiFacility = currency_partnership_smi_facility?.value || 'IDR';
        ...
        <RowWrapper sx={{ alignItems: 'center', gap: 1 }}>
          ...
          <Currency
            ...
            label=""
            placeholder={`Input ${getLabel('partnership_smi_facility', 'Syirkah SMI/Nilai Fasilitas Pembiayaan')}`}
            containerSx={{ flex: 1 }}
            value={{ currency: currency_partnership_smi_facility?.value, value: partnership_smi_facility.value }}
            onCurrencyChange={(val) => {
              const isBothIdr = val === 'IDR' && currency_partnership_customer.value === 'IDR';
              const fallbackRate = currencyDropdownList.find((dt) => dt.value === 'USD')?.rate;
              ...
              if (isBothIdr) {
                ...
                const finalRate = (val === 'USD' || currency_partnership_customer.value === 'USD')
                  ? (fallbackRate || '1')
                  : '1';
      ...
      <Box sx={{ position: 'relative' }}>
              ...
              title={
                (() => {
                  ...
                  if (isExisting) {
                    ...
                    // Calculate current input values in both currencies using current exchange rate
                    if (currentCurrencyCustomer === 'USD') {
                      currentUsdValue = currentPartnershipCustomer;
                      // Current IDR = USD * exchange rate
                      currentIdrValue = exchangeRateFromApi > 0
        ...
        <RowWrapper sx={{ alignItems: 'center', gap: 1 }}>
          ...
          <Currency
            currencyList={currencyDropdownList}
            label=""
            placeholder={`Input ${getLabel('partnership_customer', 'Syirkah Nasabah/Mitra Syarik SMI')}`}
            containerSx={{ flex: 1 }}
            value={{ currency: currency_partnership_customer?.value, value: partnership_customer.value }}
            onCurrencyChange={(val) => {
              const isBothIdr = val === 'IDR' && currency_partnership_smi_facility.value === 'IDR';
              const fallbackRate = currencyDropdownList.find((dt) => dt.value === 'USD')?.rate;
              ...
              if (isBothIdr) {
                ...
                const finalRate = (val === 'USD' || currency_partnership_smi_facility.value === 'USD')
                  ? (fallbackRate || '1')
                  : '1';
        ...
        {currency_partnership_customer?.value === 'USD' && (
          <Currency
            currencyList={currencyDropdownList}
            label={getLabel('partnership_customer_idr', 'Syirkah Nasabah (dalam Rp)')}
            placeholder={getLabel('partnership_customer_idr', 'Syirkah Nasabah (dalam Rp)')}
            containerSx={{ flex: 1, marginTop: 3 }}
        ...
        initialProps={{
          ...
          onCurrencyChange: (val) => {
            if (val.currency === 'IDR') {
              ...
              masintonMultiChange({
                currency_hishshah_value: val,
                exchange_rate_hishshah: fallbackRate || '1',
              });
            }
          },
          placeholder: `Input ${getLabel('hishshah_value', 'Nilai Hishshah')}`,
  ...
  return (
    <>
      ...
      >
        <Input
          label={getLabel('partnership_customer_name', 'Mitra Syarik SMI')}
          type="text"
          placeholder={getLabel('partnership_customer_name', 'Mitra Syarik SMI')}
          containerSx={{ flex: 1 }}
        ...
        <Input
          ...
          type="text"
          placeholder={getLabel('mmq_object', 'Objek MMQ')}
          containerSx={{ flex: 1 }}
      ...
      >
        {(currency_partnership_smi_facility?.value === 'USD' || currency_partnership_customer?.value === 'USD') &&
          <Currency
            isMandatory={!showTooltips}
            currencyList={currencyDropdownList}
            label={getLabel('exchange_rate_global', 'Konversi Mata Uang')}
            disabledCurrency
            placeholder={getLabel('exchange_rate_global', 'Konversi Mata Uang')}
            containerSx={{ alignItems: 'center', display: 'flex', flexDirection: 'row', gap: theme.spacing(3) }}
            value={{
              currency: 'IDR',
              value: exchange_rate_global.value,
            }}
            onChange={(val) => {
        ...
        >
          {
            ...
            <Box sx={{ position: 'relative' }}>
              <Box sx={{ alignItems: 'center', display: 'flex', gap: 1, mb: 1 }}>
                <TextStyle variant="body4" weight={600} color={theme.palette.custom.text}>
                  {getLabel('total_partnership', 'Total Syirkah')}
                </TextStyle>
                {(existing || facilityId) && financingFacilityData && showTooltips && (
                  <Tooltip
                    slotProps={{
...

          {
            ...
            <Currency
              currencyList={currencyDropdownList}
              label={getLabel('total_partnership_idr', 'Total Syirkah (dalam Rp)')}
              placeholder={getLabel('total_partnership_idr', 'Total Syirkah (dalam Rp)')}
          ...
          >
            <Box sx={{ alignItems: 'center', display: 'flex', gap: 1, mb: 1 }}>
              ...
              <TextStyle variant="body4" color="#D07C1B" sx={{ lineHeight: 1.5 }}>
                {(!isPartnershipSmiFacilityUnchanged || !isPartnershipCustomerUnchanged)
                  ? 'Checkbox pada Syirkah SMI dan Syirkah Nasabah akan aktif apabila terdapat perubahan nominal dari nilai existing'
