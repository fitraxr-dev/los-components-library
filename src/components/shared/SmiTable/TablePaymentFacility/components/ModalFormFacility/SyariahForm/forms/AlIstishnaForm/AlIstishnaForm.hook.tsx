...
const useAlIstishna = (props: SyariahFormsProps) => {
  ...
  useEffect(() => {
    if (financingFacilityData?.id && facilityId) {
      ...
      // Use timeout to avoid potential race condition with form reset or other effects
  ...
  useEffect(() => {
      ...
      const initExchangeRate = (
        ...
        if (isIdr) {
          ...
          if (finalRate && finalRate !== masintonForm[field].value) {
            masintonChange(field, finalRate);
          }
        }
      };

      initExchangeRate('exchange_rate_purchase_price', 'currency_purchase_price', 'exchange_rate_purchase_price');
      initExchangeRate('exchange_rate_istishna_margin', 'currency_istishna_margin', 'exchange_rate_istishna_margin');
      initExchangeRate('exchange_rate_down_payment', 'currency_down_payment', 'exchange_rate_down_payment');
      initExchangeRate('exchange_rate_istishna_installment', 'currency_istishna_installment', 'exchange_rate_istishna_installment');
    }
  }, [
    ...
    exchange_rate_istishna_margin.value,
    currency_istishna_margin.value,
    exchange_rate_down_payment.value,
    currency_down_payment.value,
    exchange_rate_istishna_installment.value,
