...
const PopupFilterV3 = ({
  ...
  const handleChangeFilterAreaProyek = (areaFilter: FilterArea) => {
    if (localValue.filter?.city) {
      setLocalValue((prevState) => {
        const newFilter = { ...prevState };
        delete newFilter.filter.city;
    ...
    if (localValue.filter?.district) {
      setLocalValue((prevState) => {
        const newFilter = { ...prevState };
        delete newFilter.filter.district;
    ...
    if (localValue.filter?.province) {
      setLocalValue((prevState) => {
        const newFilter = { ...prevState };
        delete newFilter.filter.province;
  ...
  const renderContent = (content) => {
    switch (content.type) {
      ...
      case 'area-proyek':
        return (
          <InputAreaProyek
            onChange={handleChangeFilterAreaProyek}
            city={localValue.filter?.city ? localValue.filter.city : null}
            district={localValue.filter?.district ? localValue.filter.district : null}
            province={localValue.filter?.province ? localValue.filter.province : null}
          />
        );
      default:
        return null;
