...
import NumberFormatBase from './NumberFormatter';
...
export function format<BaseType = InputAttributes>(
  ...
  if (maxLength) {
    const length = maxLength - (prefix + suffix).length;
    if (length <= 0) {
      return numStr;
    }

    beforeDecimal = beforeDecimal.slice(0, length);
  }
...
export function removeFormatting<BaseType = InputAttributes>(
  ...
  const stripNegation = (value: string, start: number, end: number) => {
    ...
    return { end, hasNegation, start, value };
  };
  ...
  // if the changed range is from suffix in the updated value, ->
  // -> and the the suffix starts with the same characters, allow the change
  ...
  const { beforeDecimal, afterDecimal, addNegation } = splitDecimal(value, allowNegative);
...
export default function NumericFormat<BaseType = InputAttributes>(
  ...
  return <NumberFormatBase {...numericFormatProps} inputProps={{ style: { backgroundColor: (props.dataChanges || props.hasDataMaster) && '#FCE6E8' } }} />;
}
