...
import type { FormatInputValueFunction, NumberFormatBaseProps, InputAttributes, Timeout } from './types';
...
function NumberFormatBase<BaseType = InputAttributes>(
  ...
  const setCaretPositionInfoBeforeChange = (el: HTMLInputElement, endOffset: number = 0) => {
    ...
    caretPositionBeforeChange.current = { selectionEnd: selectionEnd + endOffset, selectionStart };
  };
  ...
  const inputPropsInternal = Object.assign({ inputMode }, otherProps, {
    ...
    placeholder: placeholder || 'Masukan Angka',
    sx: style,
    type,
  ...
  if (displayType === 'text') {
    ...
    /* @ts-ignore */
    return <CustomInput {...inputPropsInternal} ref={getInputRef} />;
  }

  return <TextField {...inputPropsInternal} inputProps={inputProps} inputRef={getInputRef} />;
}
