...
import type { NumberFormatBaseProps, FormatInputValueFunction, OnValueChange, IsCharacterSame } from './types';
...
export function fixLeadingZero(value?: string) {
  if (!value) return;
  if (value === '0') return '0';
  ...
  const [intRaw, fracRaw = ''] = parts.split('.');
  const int = intRaw.replace(/^0+(?=\d)/, '');
  const normalizedInt = int.length ? int : (fracRaw ? '0' : '0');
  const out = fracRaw.length > 0 ? `${normalizedInt}.${fracRaw}` : normalizedInt;
...
export const findChangeRange = memoizeOnce((prevValue: string, newValue: string) => {
  ...
  return {
    from: { end: prevLength - j, start: i },
    to: { end: newLength - j, start: i },
  };
});
...
export const findChangedRangeFromCaretPositions = (
  ...
  return {
    from: { end: lastCaretPositions.selectionEnd, start: startPosition },
    to: { end: currentCaretPosition, start: startPosition },
  };
};
