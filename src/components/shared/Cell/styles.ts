...
export const styText = (type) => ({
  ...
  WebkitLineClamp: '1',
  display: type === 'text' ? '-webkit-box' : 'flex',
  ml: 1,
  overflow: type === 'text' ? 'hidden' : undefined,
  textDecoration: type === 'link' ? 'underline' : undefined,
  textWrap: type === 'text' ? 'wrap' : undefined,

  whiteSpace: type === 'buttons' ? 'normal' : 'nowrap',
  wordBreak: type === 'buttons' ? 'break-word' : 'break-all',
});
