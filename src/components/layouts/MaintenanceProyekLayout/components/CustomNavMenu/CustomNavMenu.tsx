            ...
            return (
              <Fragment key={index}>
                {root?.subMenu?.length > 0 ?
                  <CollapseMenu menu={root} checkStepperRed={checkStepperRed} /> :

                  <Box
                    display="flex"
                    sx={{
                      // Use a specific shade or color from the palette
                      backgroundColor: root.enable === false
                        ? theme.palette.action.disabledBackground
                        : pathname.includes(root?.url)
                          ? theme.palette.primary.main
                          : theme.palette.white.main,
                      border: '1px solid #284A63',
                      borderRadius: '0.5208333333333334vw',
                      // Use a specific shade for the text color
                      color: root.enable === false
                        ? theme.palette.action.disabled
                        : pathname.includes(root?.url)
                          ? theme.palette.white.main
                          : theme.palette.primary.main,
                      cursor: root.enable ? 'pointer' : 'not-allowed',
                      opacity: root.enable ? 1 : 0.5,
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          <TextStyle
                            variant="body4"
                            color={pathname.includes(root?.url) ? theme.palette.white.main : theme.palette.primary.main}
                            weight={600}
                          >
                            {root?.label}
                          </TextStyle>
