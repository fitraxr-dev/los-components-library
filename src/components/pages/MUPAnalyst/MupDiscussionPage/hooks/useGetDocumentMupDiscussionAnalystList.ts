...
const useGetDocumentMupDiscussionAnalystList = (
  ...
  const query = useQuery({
    queryFn: async () => {
      const res = await api.listDocsMipDiscussionAnalyst(payload);

      return res.data.data;
    },

    queryKey: ['document-mup-discussion-analyst-list', payload],
    ...config,
  });
  return query;
};

export default useGetDocumentMupDiscussionAnalystList;
