import { useEffect, useMemo, useState } from 'react';

import NiceModal from '@ebay/nice-modal-react';
import { yupResolver } from '@hookform/resolvers/yup';
import { useQueryClient } from '@tanstack/react-query';
import { useParams, usePathname, useRouter, useSearchParams } from 'next/navigation';
import { useForm, useFormContext } from 'react-hook-form';

import { MODAL } from '@/configs/constants/modalId';
import { userManagement } from '@/configs/constants/pathname';
import { ActivityType } from '@/enums/Activity';
import { TypeModule, TypeProcess } from '@/enums/Module';
import { getLastPath, replacePath } from '@/helpers/navigation';
import showNiceModalV2 from '@/helpers/showNiceModalV2';
import useGetParameterList from '@/hooks/services/useGetParameterList';
import useSubmitBucket from '@/hooks/services/useSubmitBucket';
import closeNiceModal from '@/hooks/useCloseNiceModal';
import useRecordLog from '@/hooks/useRecordLog';

import { modal } from '../../constants';
import useGetAccessMenuDetail from '../../hooks/useGetAccessMenuDetail';
import useGetMasterAccessMenu from '../../hooks/useGetMasterAccessMenu';
import useGetMasterDirectorate from '../../hooks/useGetMasterDirectorate';
import useGetMasterDivision from '../../hooks/useGetMasterDivision';
import useGetMasterGroup from '../../hooks/useGetMasterGroup';
import useGetMasterPosition from '../../hooks/useGetMasterPosition';
import useGetMasterRole from '../../hooks/useGetMasterRole';
import useGetUserSearch from '../../hooks/useGetUserSearch';
import useSaveUser from '../../hooks/useSaveUser';
import useValidateUser from '../../hooks/useValidateUser';

import { formDefaultValues } from './InternalCreationForm.constants';
import { getNonMandatoryYupSchema, getYupSchema } from './InternalCreationForm.schema';

import type { InternalCreationFormProps } from './InternalCreationForm.types';
import type { AccessMenu } from '../../../components/TableAccessMenu/TableAccessMenu.types';
import type * as yup from 'yup';


interface AccessMenuItems extends AccessMenu {
  subMenu: AccessMenuItems[];
}

const useInternalCreationForm = (props: InternalCreationFormProps) => {
  const { detailUser = { data: [], isLoading: false, isSuccess: false }, countResetData } = props;
  const path = usePathname();
  const router = useRouter();
  const searchParams = useSearchParams();
  const { id }: { id: string } = useParams();
  const formMethodContext = useFormContext();
  const [searchDetailValue, setSearchDetailValue] = useState({
    accessMenu: '',
    directorate: '',
    division: '',
    position: '',
    reason: '',
    reportTo: '',
    userGroup: '',
  });

  const queryClient = useQueryClient();

  const isEdit = getLastPath(path) === 'edit';
  const isAdd = getLastPath(path) === 'add';

  const yupSchema = getYupSchema(isEdit);

  const { control, handleSubmit, reset, watch, setValue, formState: { errors, isValid } } = useForm({
    defaultValues: {
      ...formDefaultValues,
      reason: isEdit
        ? { id: '', label: '' }
        : { id: 'Pendaftaran Baru', label: 'Pendaftaran Baru' },
    },
    mode: 'onChange',
    reValidateMode: 'onChange',
    resolver: yupResolver(yupSchema),
  });

  const isDetailSuccess = detailUser.isSuccess;
  const detailData = isDetailSuccess && detailUser.data;
  const { email, userType } = formMethodContext?.watch();
  const processIdFromParams = id && id.includes('UM-') ? id : '';
  const isHasProcessIdParams = id;
  const isHasProcessId = id && id.includes('UM-');
  const bucketProcessIdFromQS = searchParams.get('bucketProcessId');
  const processId = isAdd ? bucketProcessIdFromQS : processIdFromParams;
  const isReportToMandatory = watch('role') === 'STAFF' || watch('role') === 'TL';
  const [newBucketProcessId, setNewBucketProcessId] = useState<string | undefined>(undefined);
  const isHasBucketProcessId = newBucketProcessId && newBucketProcessId.includes('UM-');
  const { recordActivity } = useRecordLog();
  const [accessMenuId, setAccessMenuId] = useState<string>('');

  const { mutate: mutateValidateUser } = useValidateUser({
    onError: (error) => {
      showNiceModalV2({
        title: error.response.data.errorDetail,
        type: 'error',
      });
    },

  });

  const { data: reasonData } = useGetParameterList('userReason');

  const { data: userGroupData } = useGetMasterGroup({
    filter: {
      type: userType,
    },
    page: {
      itemPerPage: 1000000,
      noPage: 1,
    },
    searchDetail: {
      key: 'name',
    },
  });

  const { data: accessMenuData } = useGetMasterAccessMenu({
    page: {
      itemPerPage: 10000,
      noPage: 1,
    },
    searchDetail: {
      key: 'permissionName',
      value: searchDetailValue.accessMenu,
    },
  });

  const { data: roleData, isSuccess: isRoleSuccess } = useGetMasterRole({
    filter: {
      group: watch('userGroup.id'),
    },
    page: {
      itemPerPage: 1000,
      noPage: 1,
    },
  }, {
    enabled: !!watch('userGroup.id'),
  });

  const {
    data: directorateData,
    isSuccess: isDirectorateSuccess,
  } = useGetMasterDirectorate({
    filter: {
      type: userType,
    },
    page: {
      itemPerPage: 10000,
      noPage: 1,
    },
    searchDetail: {
      key: 'name',
      value: searchDetailValue.directorate,
    },
  }, {
    enabled: !!watch('role'),
  });

  const { data: divisionData, isSuccess: isDivisionSuccess } = useGetMasterDivision({
    filter: {
      directorate: watch('directorate.id'),
    },
    page: {
      itemPerPage: 10000,
      noPage: 1,
    },
    searchDetail: {
      key: 'name',
      value: searchDetailValue.division,
    },
  }, {
    enabled: !!watch('directorate.id'),
  });

  const { data: positionData, isSuccess: isPositionSuccess } = useGetMasterPosition({
    filter: {
      division: watch('division.id'),
      userType,
    },
    page: {
      itemPerPage: 10000,
      noPage: 1,
    },
    searchDetail: {
      key: 'name',
      value: searchDetailValue.position,
    },
  }, {
    enabled: !!watch('division.id'),
  });

  const isUserSearchEnable =
    (watch('role') === 'STAFF' || watch('role') === 'TL') &&
    !!watch('division.id');

  const superiorRole = useMemo(() => {
    return roleData?.contents?.find((item) => item.key === watch('role'))?.additionalData?.superiorRole;
  }, [roleData, watch('role')]);

  const { data: userSearchData, isSuccess: isUserSearchSuccess } = useGetUserSearch({
    division: watch('division.id'),
    role: superiorRole,
    value: searchDetailValue.reportTo,
  }, {
    enabled: isUserSearchEnable,
  });

  const { data: accessMenuDetailData } = useGetAccessMenuDetail(
    {
      id: accessMenuId,
    },
    {
      enabled: !!accessMenuId,
    }
  );


  const accessMenuItems = accessMenuDetailData?.menuItems as AccessMenuItems[] || [];

  const { mutate: saveUser } = useSaveUser({
    onError: (error) => {
      const errorMessage = error?.response?.data?.errorDetail || 'Data gagal disimpan';

      showNiceModalV2({
        title: errorMessage,
        type: 'error',
      });
    },
    onSuccess: (response, payload) => {
      recordActivity({
        activity: isEdit ? ActivityType.EDIT : ActivityType.ADD,
        bucketProcessId: processId,
        changeAfter: JSON.stringify(payload),
        module: TypeModule.USER_MANAGEMENT,
        process: TypeProcess.USER_MANAGEMENT,
        remarks: isEdit ? 'edit user' : 'add user',
      });
      setNewBucketProcessId(response.bucketProcessId);
      showNiceModalV2({
        onClose() {
          if (isAdd) {
            router.replace(replacePath(userManagement.USER_LIST.EDIT, { id: response.bucketProcessId }));
          }
          if (isEdit) {

            if (!isHasProcessId) {
              router.replace(replacePath(userManagement.USER_LIST.EDIT, { id: response.bucketProcessId }));
            }
          }
        },
        title: 'Data berhasil disimpan',
        type: 'success',
      });
    },
  });

  const { mutate: submitBucket } = useSubmitBucket({
    onError: () => {
      showNiceModalV2({
        title: 'Data gagal disubmit',
        type: 'error',
      });
    },
    onSuccess: (variable) => {
      recordActivity({
        activity: ActivityType.SUBMIT,
        bucketProcessId: processId,
        changeAfter: JSON.stringify(variable),
        module: TypeModule.USER_MANAGEMENT,
        process: TypeProcess.USER_MANAGEMENT,
        remarks: 'Submit Data User',
      });
      queryClient.invalidateQueries({ queryKey: ['um-user-submission-list']});
      showNiceModalV2({
        onClose() {
          router.replace(userManagement.USER_LIST.BUCKET_LIST);
        },
        title: 'Data berhasil disimpan',
        type: 'success',
      });
    },
  });

  const roleList = isRoleSuccess ? roleData?.contents?.map((role) => ({ label: role.label, value: role.key })) : [];

  const directorateList = isDirectorateSuccess ? directorateData?.contents?.map((directorate) => ({
    id: directorate.key,
    label: directorate.label,
  })) : [];

  const divisionList = isDivisionSuccess ? divisionData?.contents?.map((division) => ({
    id: division.key,
    label: division.label,
  })) : [];

  const positionList = isPositionSuccess ? positionData?.contents?.map((position) => ({
    label: position.label,
    value: position.key,
  })) : [];

  const userSearchList = isUserSearchSuccess ? userSearchData?.map((user) => ({
    id: user.userId,
    label: user.fullName,
  })) : [];

  const userGroupList = userGroupData?.contents?.map((user) => ({
    id: user.key,
    label: user.label,
  }));

  const accessMenuList = accessMenuData?.contents?.map((access) => ({
    id: access.key,
    label: access.label,
  }));

  const reasonList = reasonData?.map((reason) => ({
    id: reason.value,
    label: reason.label,
  }));

  const userStatusList = [{
    label: 'Active',
    value: 'ACTIVE',
  }, {
    label: 'Inactive',
    value: 'IN_ACTIVE',
  }];

  useEffect(() => {
    if (formMethodContext.control._options.context?.validateData?.fullName) {
      setValue('name', formMethodContext.control._options.context?.validateData?.fullName);
    }
  }, [formMethodContext]);

  useEffect(() => {
    if (isEdit && isDetailSuccess) {


      recordActivity({
        activity: ActivityType.VIEW,
        bucketProcessId: processId,
        module: TypeModule.USER_MANAGEMENT,
        process: TypeProcess.USER_MANAGEMENT,
        remarks: 'view user management detail page',
      });

      const reason = isEdit
        ? { id: detailData?.userAccount?.reason, label: detailData?.userAccount?.reason }
        : detailData?.userAccount?.reason;


      reset({
        accessMenu: {
          id: detailData?.permission?.key,
          label: detailData?.permission?.label,
        },
        description: detailData?.description,
        directorate: {
          id: (detailData?.statusLabel === 'Inactive' && detailData?.accessManagements?.[0])
            ? detailData?.accessManagements?.[0]?.userDivision?.directorate?.directorateCode
            : detailData?.accessManagementActive?.userDivision?.directorate?.directorateCode,
          label: (detailData?.statusLabel === 'Inactive' && detailData?.accessManagements?.[0])
            ? detailData?.accessManagements?.[0]?.userDivision?.directorate?.name
            : detailData?.accessManagementActive?.userDivision?.directorate?.name,
        }, division: {
          id: (detailData?.statusLabel === 'Inactive' && detailData?.accessManagements?.[0])
            ? detailData?.accessManagements?.[0]?.userDivision?.divisionCode
            : detailData?.accessManagementActive?.userDivision?.divisionCode,
          label: (detailData?.statusLabel === 'Inactive' && detailData?.accessManagements?.[0])
            ? detailData?.accessManagements?.[0]?.userDivision?.name
            : detailData?.accessManagementActive?.userDivision?.name,
        },
        expiredDate: detailData?.userAccount?.expiredDate,
        institute: detailData?.institute,
        name: detailData?.fullName,
        nik: detailData?.nik,
        position: (detailData?.status === 'IN_ACTIVE' && detailData?.accessManagements?.[0])
          ? detailData?.accessManagements?.[0]?.userPosition?.map((item) => item.positionCode)
          : detailData?.accessManagementActive?.userPosition?.map((item) => item.positionCode),
        privyId: detailData?.privyId,
        processId: detailData?.bucketProcessId,
        proposalReference: detailData?.reference,
        reason,
        reportTo: {
          id: (detailData?.status === 'IN_ACTIVE' && detailData?.accessManagements?.[0])
            ? detailData?.accessManagements?.[0]?.userSuperior?.userId
            : detailData?.accessManagementActive?.userSuperior?.userId,
          label: (detailData?.status === 'IN_ACTIVE' && detailData?.accessManagements?.[0])
            ? detailData?.accessManagements?.[0]?.userSuperior?.fullName
            : detailData?.accessManagementActive?.userSuperior?.fullName,
        },
        role: (detailData?.status === 'IN_ACTIVE' && detailData?.accessManagements?.[0])
          ? detailData?.accessManagements?.[0]?.userRoleRefactor?.roleCode
          : detailData?.accessManagementActive?.userRoleRefactor?.roleCode,
        userGroup: {
          id: (detailData?.status === 'IN_ACTIVE' && detailData?.accessManagements?.[0])
            ? detailData?.accessManagements?.[0]?.userGroup?.code
            : detailData?.accessManagementActive?.userGroup?.code,
          label: (detailData?.status === 'IN_ACTIVE' && detailData?.accessManagements?.[0])
            ? detailData?.accessManagements?.[0]?.userGroup?.name
            : detailData?.accessManagementActive?.userGroup?.name,
        },
        userId: detailData?.userId,
        userStatus: detailData?.status,
        userType: detailData?.userType,
      });
    }
  }, [detailData]);

  useEffect(() => {
    if (countResetData > 1) {
      setValue('position', formDefaultValues.position);
      setValue('userGroup', formDefaultValues.userGroup);
      setValue('role', formDefaultValues.role);
    }
  }, [countResetData, setValue]);

  useEffect(() => {
    const currentAccessMenuId = watch('accessMenu.id');
    if (currentAccessMenuId) {
      setAccessMenuId(currentAccessMenuId);
    }
  }, [watch('accessMenu.id')]);

  const handleOnAccessMenuChange = (value: any | null) => {
    if (value?.id) {
      setAccessMenuId(value.id);
    } else {
      setAccessMenuId('');
    }
  };

  const handleOnInputChange = (inputId: keyof typeof searchDetailValue, value: string) => {
    setSearchDetailValue((prev) => ({
      ...prev,
      [inputId]: value,
    }));
  };

  const handleResetNextInputByCurrentId = (currentInputId: keyof typeof searchDetailValue | 'role') => {
    switch (currentInputId) {
      case 'userGroup':
        setValue('role', formDefaultValues.role);
        setValue('directorate', formDefaultValues.directorate);
        setValue('division', formDefaultValues.division);
        setValue('position', formDefaultValues.position);
        setValue('reportTo', formDefaultValues.reportTo);
        break;
      case 'role':
        setValue('directorate', formDefaultValues.directorate);
        setValue('division', formDefaultValues.division);
        setValue('position', formDefaultValues.position);
        setValue('reportTo', formDefaultValues.reportTo);
        break;
      case 'directorate':
        setValue('division', formDefaultValues.division);
        setValue('position', formDefaultValues.position);
        setValue('reportTo', formDefaultValues.reportTo);
        setSearchDetailValue((prev) => ({ ...prev, division: '' }));
        break;
      case 'division':
        setValue('position', formDefaultValues.position);
        setValue('reportTo', formDefaultValues.reportTo);
        break;
    }
  };

  function handleOnSave() {
    const schema = getNonMandatoryYupSchema(isEdit);
    const data = watch();
    schema.validate(data, { abortEarly: false });


    mutateValidateUser({
      email: email,
      userId: null,
      userType: userType,
    }, {
      onSuccess: () => {
        const isNonKadiv = watch('role') === 'STAFF' && watch('role') === 'TL';
        const editPayload = {
          bucketProcessId: isHasProcessId ? processId : null,
          status: data.userStatus,
          userId: data.userId,
        };

        const nonKadivPayload = {
          superiorCode: data.reportTo.id,
        };

        const payload = {
          accessMenuId: data.accessMenu?.id || null,
          description: data.description,
          directorate: data.directorate.id,
          divisionCode: data.division.id,
          email,
          fullName: data.name,
          nik: data.nik,
          position: data.position,
          privyId: data.privyId,
          reason: isEdit ? data.reason.id : 'Pendaftaran Baru' as string,
          reference: data.proposalReference,
          roleCode: data.role,
          superiorCode: data.reportTo.id,
          userGroup: data.userGroup.id,
          userType,
          withValidation: false,
          ...(isEdit && editPayload),
          ...(isNonKadiv && nonKadivPayload),
        };

        saveUser(payload);
      },
    });
  }

  function handleOnSubmit() {

    mutateValidateUser({
      email: email,
      userId: null,
      userType: userType,
    }, {
      onSuccess: () => {
        NiceModal.show(MODAL.GLOBAL.COMMENT, {
          onSave: ({ comment }) => {
            submitBucket({
              submitRequestDto: {
                action: 'SUBMIT',
                bucketProcessId: newBucketProcessId || processId,
                comment,
                module: TypeModule.USER_MANAGEMENT,
                process: TypeProcess.USER_MANAGEMENT,
              },
            });
            closeNiceModal(MODAL.GLOBAL.COMMENT);
          },
        });
      },

    });
  }

  function handleOpenAccessMenuModal() {
    NiceModal.show(modal.ACCESS_MENU, { accessMenuItems });
  }

  function handleOnCancelProcess() {
    NiceModal.show(MODAL.GLOBAL.COMMENT, {
      onSave: ({ comment }) => {
        submitBucket({
          submitRequestDto: {
            action: 'CANCELED',
            bucketProcessId: processId,
            comment,
            module: TypeModule.USER_MANAGEMENT,
            process: TypeProcess.USER_MANAGEMENT,
          },
        });
        closeNiceModal(MODAL.GLOBAL.COMMENT);
      },

    });
  }

  return {
    accessMenuList,
    control,
    directorateList,
    divisionList,
    errors,
    handleOnAccessMenuChange,
    handleOnCancelProcess,
    handleOnInputChange,
    handleOnSave,
    handleOnSubmit,
    handleOpenAccessMenuModal,
    handleResetNextInputByCurrentId,
    handleSubmit,
    isAdd,
    isEdit,
    isHasBucketProcessId,
    isHasProcessId,
    isHasProcessIdParams,
    isReportToMandatory,
    isUserSearchEnable,
    isValid,
    positionList,
    reasonList,
    roleList,
    setValue,
    superiorRole,
    userGroupList,
    userSearchList,
    userStatusList,
    watch,
  };
};

export default useInternalCreationForm;
