...
const useTabSummary = () => {
  ...
  const { data: parameterGroupDetailData } = useGetParameterGroupSubmissionDetail(
    { bucketProcessId: processId },
    { enabled: isBucketProcessId }
  );

  const isProcessFormValid = React.useMemo(() => {
    const content = parameterGroupDetailData?.content;
    const stripHtml = (html?: string) => html?.replace(/<[^>]*>/g, '').trim() ?? '';

    return !!(
      content?.itemNo &&
    content?.applicationTypeKey &&
        stripHtml(content?.itemGroup)
      ...
      showNiceModalV2({
        onClose: () => {
          closeNiceModal(MODAL.GLOBAL.SUCCESS).then(() => {
            router.push(MASTER_PARAMETER.PARAMETER_CUSTOMER_DUE_DILIGENCE_LIST_PAGE);
          });
        },
  ...
  const renderButtons = React.useCallback(() => {
    ...
    const STYLE = {
      ...
      SUBMIT: { color: 'success', disabled: !hasData || !isProcessFormValid, label: 'Submit' },
    };
  ...
  }, [group, item, subItem, stepperData, router, isProcessFormValid]);
