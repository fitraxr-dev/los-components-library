...
const useTabProcess = (watchFields: any) => {
  ...
  const handleViewItemDetail = React.useCallback((data) => {
    const nextPath = replacePath(MASTER_PARAMETER.PARAMETER_CUSTOMER_DUE_DILIGENCE_DETAIL_ITEM_PAGE, {
      id: data.id,
      mode: 'detail',
      processId,
    });
    router.push(`${nextPath}?isDetail=true`);
  }, [router, processId]);
  ...
  const handleAddItem = React.useCallback(() => {
    const nextPath = replacePath(MASTER_PARAMETER.PARAMETER_CUSTOMER_DUE_DILIGENCE_CREATE_ITEM_PAGE, {
      mode,
      processId,
    });
    router.push(nextPath);
  ...
  const { mutate: saveParameterGroup } = useSaveParameterGroupDetail({
    ...
    onSuccess: (data) => {
      recordActivity({
        ...
        menuCode: 'parameter-customer-due-diligence',
        ...
        onClose: () => {
          closeNiceModal(MODAL.GLOBAL.SUCCESS).then(() => {
            const nextPath = replacePath(
              `${MASTER_PARAMETER.PARAMETER_CUSTOMER_DUE_DILIGENCE_DETAIL_PAGE}?tab=${TAB.SUMMARY}`,
              {
                mode: 'submission',
  ...
  const handleSave = (payload) => {
    saveParameterGroup({
      ...payload,
      bucketProcessId: isBucketProcessId ? processId : undefined,
  ...
  const autoSavePayload = React.useMemo(() => () => {
    const payload = {
      ...
      module: parameterGroupDetailData?.content?.module || 'CUSTOMER_DUE_DILIGENCE',
