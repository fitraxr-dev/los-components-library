...
const ItemPage = () => {
  ...
  const title = `${id ? capitalize(mode) : 'Create'} Item`;

  const isFormDisabled = isViewOnly || !isMaker || (parameterGroupDetailData?.isEditable === false) || isDetail;
          ...
          <BaseContainer sx={{ boxShadow: 7, mt: 2 }}>
            <Table
              ...
              footer={!isFormDisabled && (
                <TableFooter disabled={!itemGroupId} onClick={() => handleOpenSubItemModal({ mode: 'add' })} />
              )}
            />
          </BaseContainer>
        </SectionTitle>
