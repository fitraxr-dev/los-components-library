...
const useItemPage = (watchFields: any) => {
  ...
  const isDetail = searchParams.get('isDetail') === 'true';
    ...
    return [
      ...TABLE_HEADER_SUBITEM,
      {
        ...
        options: () => [
          ...
          ...(isMaker && parameterGroupDetailData?.content?.isEditable && !isDetail ? [{
            iconName: 'edit',
  ...
  }, [isMaker, parameterGroupDetailData, handleOpenSubItemModal, isDetail]);
        ...
        onClose: () => {
          closeNiceModal(MODAL.GLOBAL.SUCCESS).then(() => {
            const nextPath = replacePath(
              MASTER_PARAMETER.PARAMETER_CUSTOMER_DUE_DILIGENCE_DETAIL_PAGE,
              {
                mode,
  ...
  const autoSavePayload = React.useMemo(() => () => {
    const payload = {
      ...
      module: parameterGroupItemData?.content?.module || 'CUSTOMER_DUE_DILIGENCE',
