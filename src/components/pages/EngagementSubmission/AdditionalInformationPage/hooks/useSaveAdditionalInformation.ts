...
const useSaveAdditionalInformation = ({
  ...
  const mutation = useMutation({
    ...
    onSuccess: (_, variable) => {
      queryClient.invalidateQueries({ queryKey: ['reviewer-depi-step']});
      queryClient.invalidateQueries({ queryKey: ['reviewer-depi-additional-information', { bucketProcessId: variable.bucketProcessId }]});
      onSuccess();
    },
  });
