...
const useSaveComplianceAnalysis = ({
  onSuccess = () => {},
  onError = () => {},
}) => {
  ...
  const mutation = useMutation({
    mutationFn: async (payload: purposeDetailDTO) => {
      const res = await api.saveComplianceAnalysis(
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['high-risk-compliance-analysis', {
...
export default useSaveComplianceAnalysis;
