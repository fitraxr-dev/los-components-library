...
const useDraftMemo = () => {
  ...
  const { actions, isDpop } = useCreditCheckingContext();
  ...
  const isGenerateDraftButtonEnable = actions?.hasOwnProperty('ENABLE_GENERATE_DRAFT_MEMO');

  return {
    handleOpenGenerateDraftModal,
    isGenerateDraftButtonEnable,
  };
