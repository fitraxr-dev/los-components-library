...
export const useSupportingDocumentDraftTable = () => {
  const { processId } = useIdentity();
  const [noPage, setNoPage] = useState(1);
  const [itemPerPage, setItemPerPage] = useState(5);

  const { data: attachmentListData, isLoading: attachmenListLoading } = useGetListSummaryAttachment({
    filter: {
  ...
  const handleOpenAddModal = () => {
    NiceModal.show(modal.SUPPORTING_DOCUMENT_DRAFT);
  };
