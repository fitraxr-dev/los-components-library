...
const useModalManagementDetail = (id: number | null) => {
  ...
  const [cellDataWithDetail, setCellDataWithDetail] = React.useState([
    ...
    { label: 'Jabatan', value: null },
    { label: 'DOB', value: null },
    { label: 'NPWP', value: null },
    { label: 'NPWP Document', url: null, value: null },
  ]);

  const [cellDataCreditChecking, setCellDataCreditChecking] = React.useState([
  ...
  const { data: managementListData, isSuccess } = useGetManagementDetail({
    bucketProcessId: processId,
    referenceCode: managementCode,
    summaryId: id,
  }, {
    enabled: !!processId && !!managementCode,
  });
  ...
  React.useEffect(() => {
    if (!isSuccess || !managementListData) return;
    ...
    const formattedDOB = dob ? formatDate(new Date(dob), 'DD-MM-YYYY') : '-';
    ...
    if (npwpFile) {
      ...
      data.push(
        { label: 'Document NPWP', url: null, value: '-' },
        { label: 'NPWP', value: '-' }
      );
    }
    ...
    data.push({ label: 'DOB', value: formattedDOB });
