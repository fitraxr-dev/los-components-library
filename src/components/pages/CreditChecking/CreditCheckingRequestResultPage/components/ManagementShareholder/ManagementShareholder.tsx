...
const ManagementShareholder = () => {
  ...
  const { bucketDetail, selectedDebtor } = useCreditCheckingRequestResultContext();
  const [hasAutoSwitched, setHasAutoSwitched] = useState(false);
  const newRequestPagePath = replacePath(creditChecking.DETAIL_REQUEST_PAGE, { processId });
  const isRequestMode = matchesPathname(pathname, newRequestPagePath);
  const pathSegments = pathname.split('/').filter((segment) => segment);
  const isResult = pathSegments[4] === 'result';
  const { tableType } = useCreditCheckingContext();
    ...
    <>
      <ColumnWrapper gap={3}>
        ...
        <TableDebtorInformation
          ...
          process={isRequestModule ? TypeProcess.CREDIT_CHECKING : TypeProcess.CREDIT_CHECKING_DPOP}
        />
