...
export const useModalFormFinancingFacilityOtherBank = ({ id }) => {
  ...
  const [bankNameKeyword, setBankNameKeyword] = useState(null);
  ...
  useEffect(() => {
    if (!formMethods.watch('plafond.currency') || !formMethods.watch('outstanding.currency')) {
      formMethods.setValue('outstanding', null);
    }

    if (formMethods.watch('plafond.currency') === 'USD') {
      const exchangeRate = formMethods.watch('exchangeRate.value');
      const plafondValue = multiplyNominalValues(formMethods.watch('plafond.value'), exchangeRate);
      const outstandingValue = multiplyNominalValues(formMethods.watch('outstanding.value'), exchangeRate);

      formMethods.setValue('plafondIdr.value', plafondValue);
      formMethods.setValue('outstandingIdr.value', outstandingValue);
    }

  }, [formMethods.watch('plafond'), formMethods.watch('exchangeRate'), formMethods.watch('outstanding')]);
