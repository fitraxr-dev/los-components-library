...
const ModalOtherRelationDetail = create((props: ModalOtherRelationDetailProps) => {
  const theme = useTheme();
  const modalId = modal.MODAL_OTHER_RELATION_DETAIL;
  ...
  const {
    ...
    setNoPage,
    otherRelationData,
  } = useModalOtherRelationDetail(props);
    ...
    >
      ...
      >
        {cellsDataTop.length > 0 && cellsDataTop.map((item) => {
          ...
          const documentObject = item.url && otherRelationData ? {
            ...otherRelationData,
            document: item.url,
