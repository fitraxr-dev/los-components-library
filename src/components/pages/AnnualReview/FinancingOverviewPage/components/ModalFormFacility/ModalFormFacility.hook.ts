...
const usePopupFormFacility = ({ existing, id }: any) => {
  ...
  const autoSavePayload = useMemo(() => () => {
    ...
    const basePayload = {
      ...
      bucketProcessId: processId,
      currencyOrderValue: masintonForm.currencyOrderValue?.value,
      currencyOutstanding: masintonForm.currencyOutstanding?.value,

      debtorId: debtorId ? debtorId : null,
      debtorName: masintonForm.debtorName?.value,
      exchangeRate: currencyOrderValue !== 'IDR' ? formatNumber(exchangeRate) : '1',
      facilityId: existing ? '' : financingFacilityData?.facilityId || '',
      financingObjectives: financingObjectives,
      financingSegment: financingSegment,
      governmentMandate: financingSegment === 'SYARIAH' ? tempSyariahData['government_mandate'] : masintonForm.governmentMandate?.value,
      gracePeriod: masintonForm.gracePeriod?.value || '',
      mappingFinancingSegment: mappingFinancingSegment || '',
      mappingOrderType: mappingOrderType || '',
      mappingProduct: mappingProduct || '',
      module: TypeModule.ANNUAL_REVIEW,
      orderType: masintonForm.orderType?.value,
      orderValue: formatNumber(orderValue),
      orderValueAfterExchangeRate: currencyOrderValue !== 'IDR' ? formatNumber(orderValueAfterExchangeRate) : '',
      outstanding: formatNumber(outstanding),
      process: TypeProcess.ANNUAL_REVIEW,
      product: product,
      projectId: newProjectId ? Number(newProjectId) : '',
      rates: masintonForm.rates?.value || '',
      remark: financingSegment === 'SYARIAH' ? tempSyariahData['remarks'] : masintonForm.remark?.value,
      timePeriod: masintonForm.timePeriod?.value || '',
      withdrawalPeriod: masintonForm.withdrawalPeriod?.value || '',
    };

    const finalPayload = financingSegment === 'SYARIAH' ? {
      ...basePayload,
      attributes: Object.keys(tempSyariahData)
        .filter((item) => item !== 'remarks' && item !== 'government_mandate')
        .map((item) => ({
          attributeKey: item,
          attributeLabel: '',
          attributeValue: tempSyariahData[item] ? String(tempSyariahData[item]) : '',
        })),
    } : basePayload;
  ...
  const { isFetching: isAutoSaveFetching } = useAutoSaveDraft({
    isActive: isEditMode && !!financingFacilityData,
    payload: autoSavePayload,
    url: 'bucket.financialFacility.save',
  });
