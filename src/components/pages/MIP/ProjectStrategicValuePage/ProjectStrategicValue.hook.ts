...
import useGetProjectStrategicValueById from './hooks/useGetProjectStrategicValuenById';
import useSaveProjectStrategicValue from './hooks/useSaveProjectStrategicValue';


export const useProjectStrategicValue = (container: any) => {
  ...
  const {
    data: projectStrategicValueDetail,
    isFetching: isFetchLoading,
  } = useGetProjectStrategicValueById({
  ...
  // Save
  const { isPending: isSaveLoading, mutate: saveProjectStrategicValue } = useSaveProjectStrategicValue({
    onSuccess: () => {
      // Reset dirty state
  ...
  const { isFetching: isAutoSaveFetching } = useAutoSaveDraft({
    ...
    isActive: !viewOnly && !!projectStrategicValueDetail && !!container,
