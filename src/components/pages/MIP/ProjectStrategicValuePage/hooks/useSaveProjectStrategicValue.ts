...
const useSaveProjectStrategicValue = ({
  ...
  const mutation = useMutation({
    mutationFn: async ({ bucketProcessId, description, process, module }: SaveDto) => {
      const res = await api.saveProjectStrategicValue(bucketProcessId, process, module, description);
    ...
    onSuccess: (_, variable) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['mip-project-strategic-value', { bucketProcessId: variable.bucketProcessId }]});
      onSuccess();
    },
  });
