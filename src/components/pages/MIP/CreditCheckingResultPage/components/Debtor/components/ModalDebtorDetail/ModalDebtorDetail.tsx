...
const ModalDebtorDetail = create((props: ModalDebtorDetailProps) => {
  const modalId = modal.MODAL_DEBTOR_DETAIL;
    ...
    >
      ...
      >
        {cellsDataTop.length > 0 && cellsDataTop.map((item) => {
          const fileName = 'fileName' in item && item.fileName ? item.fileName : item.value;
          const documentExtension = 'extension' in item ? item.extension : undefined;
          const documentObject = item.url && debtorData ? {
            ...debtorData,
            document: item.url,
            documentExtension,
            fileName,
