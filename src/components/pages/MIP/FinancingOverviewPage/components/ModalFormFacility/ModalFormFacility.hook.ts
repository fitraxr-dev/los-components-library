...
const usePopupFormFacility = ({ existing, id, onSuccess: onSuccessProp }: any) => {
  ...
  const { moduleType, processType } = useMemo(() => {
    const isReview = processId?.startsWith('MIPR-');
    return {
      moduleType: isReview ? TypeModule.MIP_REVIEW : TypeModule.MIP,
      processType: isReview ? TypeProcess.MIP_REVIEW : TypeProcess.MIP,
    };
  }, [processId]);
