...
const useSaveFinancingOverview = ({
  ...
  const mutation = useMutation({
    mutationFn: async ({ id, bucketProcessId, process, module, remark }: SaveDto) => {
      const res = await api.saveFinancingFacilityOverview(bucketProcessId, process, module, id, remark);
