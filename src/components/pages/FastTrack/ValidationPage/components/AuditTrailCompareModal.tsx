...
interface AuditTrailCompareModalProps {
  logId: number;
  logData?: any;
}

const AuditTrailCompareModal = NiceModal.create(({ logId, logData }: AuditTrailCompareModalProps) => {
  const theme = useTheme();
  const modal = useModal();

  if (!logData) return null;

  const header = logData?.header ?? {};
  const currentChange = logData?.currentChange ?? {};
  ...
  return (
    ...
    >
      ...
      <Box sx={{ flex: 1, mb: 3, overflowY: 'auto', pr: 1 }}>
        ...
        <TableContainer component={Paper} variant="outlined" sx={{ borderRadius: theme.radius(1), mb: 1 }}>
          <MuiTable size="small">
            <TableHead>
...
export default AuditTrailCompareModal;
