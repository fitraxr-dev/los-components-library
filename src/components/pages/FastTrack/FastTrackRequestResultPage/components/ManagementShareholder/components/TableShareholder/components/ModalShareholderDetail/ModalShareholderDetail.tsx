...
const ModalShareholderDetail = NiceModal.create(({ id, tableType }: { id: number; tableType: string }) => {
  const theme = useTheme();

  const modalId = MODAL_SHAREHOLDER.SHAREHOLDER_DETAIL;
      ...
      {(showDetailCondition || tableType === 'SUMMARY' || tableType === 'UPLOAD_RESULT') && (
        ...
        >
          ...
          {(tableType === 'SUMMARY' || showDetailCondition) && (
            ...
            >
              {cellDataFastTrack.map((item, index) => (
                <Cell
                  title={item.label}
                  value={item.value}
                  options={item.sx}
