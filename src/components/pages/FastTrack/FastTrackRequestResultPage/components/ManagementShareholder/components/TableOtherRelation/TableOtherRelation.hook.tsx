...
import useGetOtherRelationList from './hooks/useGetOtherRelationList';
import { MODAL_OTHER_RELATION, TABLE_HEADER, TABLE_HEADER_SUMMARY } from './TableOtherRelation.constants';

import type { TableHeader } from '@/components/shared/Table/Table.types';


const useTableOtherRelation = () => {
  ...
  const [selectedOtherRelationSet, setSelectedOtherRelationSet] = React.useState(() => new Set());
  ...
  const { data: otherRelationListData, isLoading } = useGetOtherRelationList({
    filter: {
  ...
  // Sync selectedOtherRelationSet with context selectedOtherRelation
  React.useEffect(() => {
    if (selectedOtherRelation && Array.isArray(selectedOtherRelation)) {
      const validSelected = selectedOtherRelation.filter((otherRelation) =>
        otherRelation && otherRelation.id !== undefined && otherRelation.id !== null
      );
      const selectedIds = new Set(validSelected.map((otherRelation) => otherRelation.id));
      setSelectedOtherRelationSet(selectedIds);
    }
  }, [selectedOtherRelation]);

  // Auto-select items that have isSelected: true in tableData
  React.useEffect(() => {
    if (hasInitializedSelection.otherRelation) return;

    if (otherRelationListData?.contents && Array.isArray(otherRelationListData.contents)) {
      initializeTableSelection('otherRelation', otherRelationListData.contents);
    }
  }, [otherRelationListData?.contents, hasInitializedSelection.otherRelation, initializeTableSelection]);

  const isAddButtonEnabled = React.useMemo(() => {
    if (!otherRelationListData?.contents || !Array.isArray(otherRelationListData.contents)) return false;

    const selectedItems = otherRelationListData.contents.filter((item) => item.isSelected === true);
    if (selectedItems.length === 0) return false;

    const atLeastOneSelectedIsDone = selectedItems.some((item) => item.isDone === true);

    return atLeastOneSelectedIsDone;
  }, [otherRelationListData?.contents]);

  const handleEditData = React.useCallback((row) => {
    sessionStorage.setItem('otherRelatedCode', row.otherRelatedCode);
    router.push(`${pathname}/edit/other-related/${row.summaryId}`);
  }, [pathname, router]);

  const handleAddData = React.useCallback(() => {
    sessionStorage.removeItem('summaryDetailId');
    router.push(`${pathname}/add/other-related/${null}`);
  }, [pathname, router]);

  const handleViewDetailData = React.useCallback((row) => {
    sessionStorage.setItem('otherRelatedCode', row.otherRelatedCode);
    NiceModal.show(MODAL_OTHER_RELATION.OTHER_RELATION_DETAIL, { id: row.summaryId, tableType });
  }, []);
  ...
  const toggleSelected = React.useCallback((row) => {
    ...
    setSelectedOtherRelation((prev) => {
      const validPrev = prev.filter((item) => item && item.id !== undefined && item.id !== null);
      const exists = validPrev.some((otherRelation) => otherRelation.id === row.id);
      if (exists) {
        const result = validPrev.filter((otherRelation) => otherRelation.id !== row.id);
  ...
  const { mutate: deleteData } = useDeleteDataShareholder({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['fast-track', 'other-related']});
    },
  });
  ...
  const tableHeader: TableHeader[] = React.useMemo(() => ([
    {
      ...
      isSelected: (row) => selectedOtherRelationSet.has(row.id),
      key: 'checkbox',
      label: '',
  ...
  ]), [actionOptions, tableHeaderCore, selectedOtherRelationSet, toggleSelected]);
  ...
  return {
    ...
    setPageSize,
    showAddButton,
    tableData: otherRelationListData?.contents,
    tableHeader,
    totalPage: otherRelationListData?.page?.totalPage,
  };
};

export default useTableOtherRelation;
