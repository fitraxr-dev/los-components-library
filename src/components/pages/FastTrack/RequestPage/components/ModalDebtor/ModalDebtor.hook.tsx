...
import useRequestFastTrack from '../../hooks/useRequestFastTrack';
...
export const useModalDebtor = (modalId: string) => {
  ...
  const { data: filterActiveInOptions } = useGetParameterList('filterActiveInFastTrack');
  //END PARAMETER LIST
  ...
  const { mutate: requestFastTrack } = useRequestFastTrack({
    onError: () => {
      showNiceModalV2({
    ...
    onSuccess: (data) => {
      router.push(replacePath(
        fastTrack.REQUEST_DEBTOR_INFORMATION_PAGE,
        {
