...
import { accessid, fastTrack } from '@/configs/constants/pathname';
...
const useRequest = () => {
  ...
  const { filterStatusFastTrack } = useFastTrackContext();
  ...
  const canViewFastTrack = useCheckAccess(accessid.REQUEST_CREDIT_CHECKING_VIEW);
  ...
  const { data: statusOptions } = useGetParameterList('status');
  const { data: searchByOptions } = useGetParameterList('searchByRequestFastTrack', {
    label: 'value1',
    value: 'value2',
  });

  const { data: sortByOptions } = useGetParameterList('sortByBucketFastTrack', {
    label: 'value1',
    value: 'value2',
  });
  ...
  const { data: filterByDivisionOptions } = useGetParameterList('filterDivisionFastTrack', {
    label: 'value1',
    value: 'value2',
  });
  ...
  const tableHeader: TableHeader[] = [
    ...
    {
      key: 'totalNominalPengajuan',
      label: 'Total Nominal Pengajuan',
      render: (row) => React.createElement(TextStyle, { variant: 'body4' }, 'Rp.1.000.000.000'),
      sx: { minWidth: '10vw' },
    },
    {
      ...
      options: [
        ...(canViewFastTrack ? [{
          iconName: 'detail', onClick: (data) => {
            ...
            dispatch({
              data: { ...state.pages, fastTrackProcess: data.process },
              type: reducer.SET_PAGES,
            });
            router.push(replacePath(fastTrack.REQUEST_DEBTOR_INFORMATION_PAGE, { processId }));
          },
        }] : []),
      ],
      sx: { minWidth: '6vw' },
