...
const useModalFormMember = () => {
  ...
  const [selectedMember, setSelectedMember] = useState<Array<{ debtorId: string; debtorName: string }>>([]);
    ...
    onSuccess: (data, variables) => {
      recordActivity({
        ...
        changeAfter: JSON.stringify({
          addedMembers: selectedMember,
          debtorIds: variables.debtorCode,
        }),
        changeBefore: '',
        menuCode: 'maintenance-group',
        module: TypeModule.MAINTENANCE_GROUP,
        process: TypeProcess.MAINTENANCE_GROUP,
        remarks: `successfully added ${selectedMember.length} member(s) to maintenance group`,
      });
  ...
  const handleAddMemberToTable = (selectedDebtor: { id: string | number; label: string }) => {
    setSelectedMember([...selectedMember, {
      debtorId: selectedDebtor.id.toString(),
      debtorName: selectedDebtor.label,
    }]);
  };
  ...
  const debtorAutoCompleteList = debtorListData?.data?.contents
    ?.filter((val) => {
      return !selectedMember.some((member) => member.debtorId === val?.debtorId);
    })
    ?.map((val) => {
      return {
        id: val?.debtorId,
  ...
  return {
    ...
    isButtonDisabled: selectedMember.length === 0,
    isLoading,
    mockTableData,
    modalId,
    selectedMember,
