...
import useGetModalShareholderDetailExisting from './ModalShareholderDetailExisting.hook';


const ModalShareholderDetailExisting = NiceModal.create(({ id }: { id: number }) => {
  const modalId = modalData.MODAL_SHAREHOLDER_DETAIL_EXISTING;
  const theme = useTheme();
  const modal = useModal(modalId);

  const { cellDataWithDetail } = useGetModalShareholderDetailExisting(id);
