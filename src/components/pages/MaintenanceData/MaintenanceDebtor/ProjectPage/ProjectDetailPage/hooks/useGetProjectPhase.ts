...
const useGetProjectPhase = (payload: GenericBucketRequestDtoRequestByIdDtoLong) => {
  const query = useQuery({
    queryFn: async () => {
      const res = await api.projectPhaseAll(payload);

      return res?.data?.data;
    },
    queryKey: ['project-phase-all', payload],
...
export default useGetProjectPhase;
