...
import useSaveSyndicationInformationOther from '../../../../../ConventionalFacilityPage/hooks/SyndicationInformation/useSaveSyndicationInformationOther';
import useGetChildLimitSyndication from '../../../../hooks/useGetChildLimitSyndication';
import useGetParentLimit from '../../../../hooks/useGetParentLimit';
import useSaveSyndicationTab from '../../../../hooks/useSaveSyndicationTab';
import useSaveSyndicationTabOther from '../../../../hooks/useSaveSyndicationTabOther';
...
const useInformasiSindikasi = () => {
  ...
  useEffect(() => {
    recordActivity({
      ...
      remarks: 'view detail manajemen fasilitas syariah informasi sindikasi',
    });
  }, []);

  const { data: dataTabSyndication } = useGetChildLimitSyndication({
  ...
  useEffect(() => {
    if (dataTabSyndication) {
      const content = dataTabSyndication;
      if (!content) return;
  ...
  const { mutateAsync: saveSyndicationTabAsync } = useSaveSyndicationTab({
    onError(error) {
      showNiceModalV2({
        onClose: () => {},
        title: error?.message,
      ...
      recordActivity({
        ...
        module: TypeModule.MAINTENANCE_DEBTOR,
        process: TypeProcess.MAINTENANCE_DEBTOR,
        remarks: 'edit data error in manajemen fasilitas syariah informasi sindikasi',
      });
    },
    onSuccess() {
      showNiceModalV2({
      ...
      recordActivity({
        ...
        module: TypeModule.MAINTENANCE_DEBTOR,
        process: TypeProcess.MAINTENANCE_DEBTOR,
        remarks: 'edit data success in manajemen fasilitas syariah informasi sindikasi',
      });
    },
  });
