...
import { listMenu } from './Menu.constants';
...
const Menu = () => {
  ...
  const activeMenu = searchParams.get('menu') || 'child-limit';
            ...
            {listMenu.map((root, index) => (
              <Fragment key={index}>
                <Box
                  display="flex"
                  sx={{
                    backgroundColor: activeMenu === root?.id ?
                      theme.palette.primary.main : theme.palette.white.main,
                    border: '1px solid #284A63',
                    borderRadius: '0.5208333333333334vw',
                    color: activeMenu === root?.id ? theme.palette.white.main : theme.palette.primary.main,
                    cursor: 'pointer',
                    ...
                    >
                      ...
                      >
                        <TextStyle
                          variant="body4"
                          color={
                            activeMenu === root?.id ? theme.palette.white.main : theme.palette.primary.main
                          }
                          weight={600}
                        >
                          {root?.label}
