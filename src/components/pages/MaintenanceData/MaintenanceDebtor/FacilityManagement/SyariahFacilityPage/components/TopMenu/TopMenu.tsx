...
const TopMenu = ({ idInduk, type = 'limit-induk' }: TopMenuProps) => {
  const {
    handleClickMenu,
    lastPath,
    listMenu,
    theme,
  } = useTopMenu({ idInduk, type });
            ...
            {listMenu.map((root, index) => (
              <Fragment key={index}>
                <Box
                  display="flex"
                  sx={{
                    backgroundColor: lastPath === root?.lastPath ?
                      theme.palette.primary.main : theme.palette.white.main,
                    border: '1px solid #284A63',
                    borderRadius: '0.5208333333333334vw',
                    color: lastPath === root?.lastPath ? theme.palette.white.main : theme.palette.primary.main,
                    cursor: lastPath !== root?.lastPath && 'pointer',
                    justifyContent: 'center',
                    paddingX: theme.spacing(3),
                    position: 'relative',
                  }}
                  key={index}
                  onClick={() => { lastPath !== root?.lastPath && handleClickMenu(root); }}
                >
                  <Box
                    ...
                    >
                      ...
                      >
                        <TextStyle
                          variant="body4"
                          color={
                            lastPath === root?.lastPath ? theme.palette.white.main : theme.palette.primary.main
                          }
                          weight={600}
                        >
                          {root?.label}
