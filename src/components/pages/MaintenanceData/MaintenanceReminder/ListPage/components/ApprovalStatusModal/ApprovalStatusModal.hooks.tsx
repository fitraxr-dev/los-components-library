...
import useGetReminderSubmission from '../../hooks/useGetReminderSubmission';
...
const useApprovalStatusModal = () => {
  ...
  const { data: searchByOptions } = useGetParameterList('searchByReminderApprovalStatus');
  const { data: sortByOptions } = useGetParameterList('sortByReminderApprovalStatus');
  const { data: statusOptions } = useGetParameterList('reminderStatus');
  ...
  const { data: submissionData, isFetching: isLoading } = useGetReminderSubmission({
    filter: {
      ...filter?.filter,
          ...
          onClick: (data) => {
            ...
            if (isMAKER && data?.createdBy === userId) {
              if (data?.status === 'MAINTENANCE DATA') {
                nextPath = replacePath(
                  `${maintenanceReminder.DETAIL_PAGE}?action=edit&flow=maintenance-data`,
                  { id: data?.bucketProcessId }
                );
              } else if (data?.status === 'RETURN TO MAKER') {
                // decline, save, submit
                nextPath = replacePath(
                  `${maintenanceReminder.DETAIL_PAGE}?action=edit&flow=return-maker`,
                  { id: data?.bucketProcessId }
                );
              }
            }
            ...
            if (!nextPath) {
              if (isCHECKER && data?.status === 'WAITING APPROVAL CHECKER') {
                ...
                nextPath = replacePath(
                  `${maintenanceReminder.DETAIL_PAGE}?action=edit&flow=waiting-approval`,
                  { id: data?.bucketProcessId }
                );
              } else {
                nextPath = replacePath(
                  `${maintenanceReminder.DETAIL_PAGE}?action=detail-from-approval`,
                  { id: data?.bucketProcessId }
                );
              }
            }
