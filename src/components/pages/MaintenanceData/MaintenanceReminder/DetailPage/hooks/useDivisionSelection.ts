...
import type { ReminderReceiverResponseDto } from '@/services/openapi/notification-service';
    ...
    const divisionRoleMap = apiReceivers.reduce((acc, curr) => {
      if (!acc[curr.divisionCode ?? '']) {
        acc[curr.divisionCode ?? ''] = {
          ...
          divisionName: curr.divisionName ?? '', // pastikan API ada divisionName
          roles: [],
        };
      }
      acc[curr.divisionCode ?? ''].roles.push({
        ...
        positionName: curr.positionName ?? '', // pastikan API ada positionName
        selected: curr.isActive ?? false,
      });
      return acc;
    }, {} as Record<string, {
      divisionCode: string;
