...
const useTabHistoryDownload = () => {
  ...
  const { data, isLoading, refetch } = useGetHistoryReportBmppIndividualDpop({
    page: {
  ...
  const { mutate: downloadReport, isPending: isDownloading } = useDownloadReportBmppIndividualDpop({
    onError: (error) => {
    ...
    onSuccess: (response, id) => {
      ...
      try {
        ...
        recordActivity({
          ...
          remarks: 'download file report bmpp individual dpop',
        });
      } catch (error) {
  ...
  const getActionOptions = (row: any) => {
    ...
    switch (status) {
      case 'success':
        return [
          {
            apiDownload: 'report.bmppIndividualDpop.download',
            iconName: 'download',
