...
import useDownloadReportLaporanCustomerProject from '@/hooks/services/report/laporan-detail-customer-project/useDownloadReportLaporanDetailCustomerProject';
import useGenerateHistoryLaporanCustomerProject from '@/hooks/services/report/laporan-detail-customer-project/useGenerateHistoryLaporanDetailCustomerProject';
...
const useTabHistoryDownload = () => {
  ...
  // user access
  const canDownloadFile = useCheckAccess(accessid.REPORT_DETAIL_CUSTOMER_PROJECT_DOWNLOAD);
  ...
  const { data, isLoading, refetch } = useGenerateHistoryLaporanCustomerProject({
    page: {
  ...
  const { mutate: downloadReport, isPending: isDownloading } = useDownloadReportLaporanCustomerProject({
    onError: (error) => {
    ...
    onSuccess: (response, id) => {
      ...
      recordActivity({
        ...
        remarks: 'download file report detail customer project',
      });
  ...
  const getActionOptions = (row: any) => {
    ...
    switch (status) {
      case 'success':
        return [
          {
            apiDownload: 'report.customerProject.download',
            iconName: 'download',
