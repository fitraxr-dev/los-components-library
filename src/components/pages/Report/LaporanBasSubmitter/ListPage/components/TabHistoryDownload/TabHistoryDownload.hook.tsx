...
import useDownloadReportBasAsSubmitter from '@/hooks/services/report/bas-as-submitter/useDownloadReportBasAsSubmitter';
import useGenerateHistoryBasAsSubmitter from '@/hooks/services/report/bas-as-submitter/useGenerateHistoryBasAsSubmitter';
...
const useTabHistoryDownload = () => {
  ...
  const { data, isLoading, refetch } = useGenerateHistoryBasAsSubmitter({
    page: {
  ...
  const { mutate: downloadReport, isPending: isDownloading } = useDownloadReportBasAsSubmitter({
    onError: (error) => {
    ...
    onSuccess: (response, id) => {
      ...
      try {
        ...
        recordActivity({
          ...
          remarks: 'download file report laporan bas as submitter',
        });
      } catch (error) {
  ...
  const getActionOptions = (row: any) => {
    ...
    switch (status) {
      case 'success':
        return [
          {
            apiDownload: 'report.basAsSubmitter.download',
            iconName: 'download',
