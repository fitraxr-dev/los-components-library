...
import useGenerateReportExcelBasAsSubmitter from '@/hooks/services/report/bas-as-submitter/useGenerateReportExcelBasAsSubmitter';
import useGenerateReportPDFBasAsSubmitter from '@/hooks/services/report/bas-as-submitter/useGenerateReportPDFBasAsSubmitter';
import useGetDataReportBasAsSubmitter from '@/hooks/services/report/bas-as-submitter/useGetDataReportBasAsSubmitter';
...
const useTabListData = () => {
  ...
  const canDownloadFile = useCheckAccess(accessid.LAPORAN_BAS_SUBMITTER_DOWNLOAD);
  ...
  const { data: summaryOptions = []} = useGetParameterList('summaryBasAsSubmitter');
  const { data: statusOptions = []} = useGetParameterList('statusBasAsSubmitter');
  ...
  // Get report data
  const { data, isLoading: isLoadingQuery, isFetching, error } = useGetDataReportBasAsSubmitter(
    searchParams !== null ? {
      filter: {
        ...searchParams,
      },
  ...
  const { mutate: generateExcel } = useGenerateReportExcelBasAsSubmitter({
    onError: () => {
      showNiceModalV2({
    ...
    onSuccess: (data) => {
      ...
      recordActivity({
        ...
        remarks: 'generate download excel report laporan bas as submitter',
      });
    },
  });

  const { mutate: generatePDF } = useGenerateReportPDFBasAsSubmitter({
    onError: () => {
      showNiceModalV2({
    ...
    onSuccess: (data) => {
      ...
      recordActivity({
        activity: ActivityType.DOWNLOAD,
        remarks: 'generate download pdf report laporan bas as submitter',
      });
    },
  });
  ...
  const tableHeader: TableHeader[] = [
    ...
    },
    {
      ...
      key: 'initiator',
      label: 'Initiator',
      onSort: () => handleSort('initiator'),
      sortDirection: sort === 'initiator' ? order : false,
      sx: { minWidth: '10vw' },
    },
    {
      isSortable: true,
