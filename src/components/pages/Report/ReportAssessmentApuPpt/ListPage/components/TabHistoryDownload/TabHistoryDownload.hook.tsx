...
import useDownloadReportApuPpt from '@/hooks/services/report/assessment-apu-ppt/useDownloadReportApuPpt';
import useGenerateHistoryApuPpt from '@/hooks/services/report/assessment-apu-ppt/useGenerateHistoryApuPpt';
...
const useTabHistoryDownload = () => {
  ...
  const { data, isLoading, refetch } = useGenerateHistoryApuPpt({
    page: {
  ...
  const { mutate: downloadReport, isPending: isDownloading } = useDownloadReportApuPpt({
    onError: (error) => {
      showNiceModalV2({
        title: error?.message,
        type: 'error',
    ...
    onSuccess: (response, id) => {
      ...
      try {
        ...
        recordActivity({
          ...
          remarks: 'download file report assessment apu ppt',
        });
      } catch (error) {
  ...
  const getActionOptions = (row: any) => {
    ...
    switch (status) {
      case 'success':
        return [
          {
            apiDownload: 'report.assessmentApuPpt.download',
            iconName: 'download',
