...
import useGenerateReportExcelBmppGroup from '@/hooks/services/report/report-bmpp-group/useGenerateReportExcelBmppGroup';
import useGenerateReportPDFBmppGroup from '@/hooks/services/report/report-bmpp-group/useGenerateReportPDFBmppGroup';
import useGetDataReportBmppGroup from '@/hooks/services/report/report-bmpp-group/useGetDataReportBmppGroup';
import useGetGroupList from '@/hooks/services/report/report-bmpp-group/useGetGroupList';
...
const useTabListData = () => {
  ...
  const canDownloadFile = useCheckAccess(accessid.BMPP_GROUP_DOWNLOAD);
  ...
  // Get report data
  const { data, isFetching, error } = useGetDataReportBmppGroup(
    searchParams !== null ? {
      filter: {
        ...searchParams,
      },
  ...
  const { mutate: generateExcel } = useGenerateReportExcelBmppGroup({
    onError: (error: any) => {
    ...
    onSuccess: (data) => {
      ...
      recordActivity({
        ...
        remarks: 'generate download excel report bmpp group',
      });
    },
  });

  const { mutate: generatePDF } = useGenerateReportPDFBmppGroup({
    onError: (error: any) => {
      console.log('error', error);
      showNiceModalV2({
    ...
    onSuccess: (data) => {
      ...
      recordActivity({
        activity: ActivityType.DOWNLOAD,
        remarks: 'generate download pdf report bmpp group',
      });
    },
  });
  ...
  const tableHeader: TableHeader[] = [
      ...
      options: [
        {
          ...
          onClick: (row: any) => {
            recordActivity({
              ...
              remarks: `view report bmpp group detail: ${row?.groupName || 'N/A'} (ID: ${row?.groupId || 'N/A'})`,
            });
    ...
    {
      isSortable: true,
      key: 'totalFasilitasUsulanGroup',
      label: 'Total Usulan Fasilitas Group',
      onSort: () => handleSort('totalFasilitasUsulanGroup'),
      sortDirection: sort === 'totalFasilitasUsulanGroup' ? order : false,
      sx: { minWidth: '12vw' },
    },
    {
      isSortable: true,
      key: 'kelonggaranBmppGroup',
      label: 'Kelonggarang BMPP terhadap Group',
  ...
  const handleViewDetail = (row: any) => {
    NiceModal.show(MODAL.REPORT.BMPP_GROUP_DETAIL, {
