...
const TabListData = () => {
  ...
  useEffect(() => {
    recordActivity({
      ...
      remarks: 'view report bmpp group',
    });
  }, []);
  ...
  return (
    <ColumnWrapper gap={theme.spacing(isMobile ? 2 : 3)}>
      ...
      >
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <Grid container spacing={isMobile ? 1.5 : 2}>
            {/* Row 1: Division (left) | Group Name (right) */}
            ...
            {/* Row 2: Group Type (left) | Group Relation (right) */}
            <Grid item xs={12} md={6}>
              <Controller
                name="groupType"
                control={control}
                render={({ field, fieldState }) => (
                  <MultipleAutoComplete
                    id="input-groupType"
                    label="Group Type"
                    placeholder="Choose Group Type(s)"
            ...
            <Grid item xs={12} md={6}>
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <MultipleAutoComplete
                    id="input-groupRelation"
                    label="Group Relation"
                    placeholder="Choose Group Relation(s)"
                    dropdownList={groupRelationOptions}
                    value={field.value || []}
                    onChange={(newValues) => {
                      field.onChange(newValues);
                    }}
      ...
      <ModalDef
        id={MODAL.REPORT.BMPP_GROUP_DETAIL}
