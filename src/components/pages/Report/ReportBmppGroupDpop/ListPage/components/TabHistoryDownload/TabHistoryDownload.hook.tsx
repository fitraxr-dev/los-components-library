...
import useDownloadReportBmppGroupDpop from '@/hooks/services/report/report-bmpp-group-dpop/useDownloadReportBmppGroupDpop';
import useGetHistoryBmppGroupDpop from '@/hooks/services/report/report-bmpp-group-dpop/useGetHistoryBmppGroupDpop';
...
const useTabHistoryDownload = () => {
  ...
  const { data, isLoading, refetch } = useGetHistoryBmppGroupDpop({
    page: {
  ...
  const { mutate: downloadReport, isPending: isDownloading } = useDownloadReportBmppGroupDpop({
    onError: (error) => {
    ...
    onSuccess: (response, id) => {
      ...
      try {
        ...
        recordActivity({
          ...
          remarks: 'download file report bmpp group dpop',
        });
      } catch (error) {
  ...
  const getActionOptions = (row: any) => {
    ...
    switch (status) {
      case 'success':
        return [
          {
            apiDownload: 'report.bmppGroupDpop.download',
            iconName: 'download',
