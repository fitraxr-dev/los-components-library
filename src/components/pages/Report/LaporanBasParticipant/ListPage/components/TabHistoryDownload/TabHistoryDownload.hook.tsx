...
import useDownloadReportBasAsParticipant from '@/hooks/services/report/bas-as-participant/useDownloadReportBasAsParticipant';
import useGenerateHistoryBasAsParticipant from '@/hooks/services/report/bas-as-participant/useGenerateHistoryBasAsParticipant';
...
const useTabHistoryDownload = () => {
  ...
  const { data, isLoading, refetch } = useGenerateHistoryBasAsParticipant({
    page: {
  ...
  const { mutate: downloadReport, isPending: isDownloading } = useDownloadReportBasAsParticipant({
    onError: (error) => {
    ...
    onSuccess: (response, id) => {
      ...
      try {
        ...
        recordActivity({
          ...
          remarks: 'download file report laporan bas as participant',
        });
      } catch (error) {
  ...
  const getActionOptions = (row: any) => {
    ...
    switch (status) {
      case 'success':
        return [
          {
            apiDownload: 'report.basAsParticipant.download',
            iconName: 'download',
