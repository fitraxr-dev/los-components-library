...
import useDownloadReportLogEndOfDay from '@/hooks/services/report/report-log-end-of-day/useDownloadReportLogEndOfDay';
import useGenerateHistoryLogEndOfDay from '@/hooks/services/report/report-log-end-of-day/useGenerateHistoryLogEndOfDay';
...
const useTabHistoryDownload = () => {
  ...
  // user access
  const canDownloadFile = useCheckAccess(accessid.REPORT_LOG_END_OF_DAY_DOWNLOAD);
  ...
  const { data, isLoading, refetch } = useGenerateHistoryLogEndOfDay({
    page: {
  ...
  const { mutate: downloadReport, isPending: isDownloading } = useDownloadReportLogEndOfDay({
    onError: (error) => {
    ...
    onSuccess: (response, id) => {
      ...
      recordActivity({
        ...
        remarks: 'download file report log end of day',
      });
  ...
  const getActionOptions = (row: any) => {
    ...
    switch (status) {
      case 'success':
        return [
          {
            apiDownload: 'report.logEndofDay.download',
            iconName: 'download',
