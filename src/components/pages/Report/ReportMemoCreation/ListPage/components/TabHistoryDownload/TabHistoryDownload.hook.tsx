...
import useDownloadReportMemoCreation from '@/hooks/services/report/report-memo-creation/useDownloadReportMemoCreation';
import useGenerateHistoryMemoCreation from '@/hooks/services/report/report-memo-creation/useGenerateHistoryMemoCreation';
...
const useTabHistoryDownload = () => {
  ...
  // user access
  const canDownloadFile = useCheckAccess(accessid.REPORT_MEMO_CREATION_DOWNLOAD);
  ...
  const { data, isLoading, refetch } = useGenerateHistoryMemoCreation({
    page: {
  ...
  const { mutate: downloadReport, isPending: isDownloading } = useDownloadReportMemoCreation({
    onError: (error) => {
    ...
    onSuccess: (response, id) => {
      ...
      recordActivity({
        ...
        remarks: 'download file report memo creation',
      });
  ...
  const getActionOptions = (row: any) => {
    ...
    switch (status) {
      case 'success':
        return [
          {
            apiDownload: 'report.memoCreation.download',
            iconName: 'download',
