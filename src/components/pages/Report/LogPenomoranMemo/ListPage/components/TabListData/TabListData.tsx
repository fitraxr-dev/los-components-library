...
const TabListData = () => {
  ...
  useEffect(() => {
    recordActivity({
      ...
      remarks: 'view report log penomoran memo',
    });
  }, []);
  ...
  const dataTable = searchParams === null ? [] : rawContents.map((item, index) => ({
    ...
    digitalDocumentDate: item.digitalDocumentDate && item.digitalDocumentDate !== 'null' ? item.digitalDocumentDate : null,
    digitalDocumentNo: item.digitalDocumentNo || '-',
    divisionReleaser: item.divisionReleaser || '-',
    documentName: item.documentName || '-',
    targetDivision: item.targetDivision || '-',
  }));
      ...
      >
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <Grid container spacing={isMobile ? 1.5 : 2}>
            ...
            {/* Row 2: Memo Name (left) | No Memo (right) */}
            <Grid item xs={12} md={6} style={{ paddingTop: isMobile ? 12 : 24 }}>
              <Controller
                name="memoName"
                control={control}
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Input
                    ...
                    placeholder="No Memo"
                    error={!!errors.noMemo}
                    helperText={errors.noMemo?.message}
                  />
                )}
              />
            </Grid>
