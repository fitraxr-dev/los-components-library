...
import useDownloadReportLogPenomoranMemo from '@/hooks/services/report/log-penomoran-memo/useDownloadReportLogPenomoranMemo';
import useGetHistoryLogPenomoranMemo from '@/hooks/services/report/log-penomoran-memo/useGetHistoryLogPenomoranMemo';
...
const useTabHistoryDownload = () => {
  ...
  // Get history data
  const { data, isLoading, refetch } = useGetHistoryLogPenomoranMemo({
    page: {
  ...
  // Download mutation
  const { mutate: downloadReport, isPending: isDownloading } = useDownloadReportLogPenomoranMemo({
    onError: (error) => {
    ...
    onSuccess: (response, id) => {
      ...
      try {
        ...
        recordActivity({
          ...
          remarks: 'download file report log penomoran memo',
        });
      } catch (error) {
  ...
  const getActionOptions = (row: any) => {
    ...
    switch (status) {
      case 'success':
        return [
          {
            apiDownload: 'report.logPenomoranMemo.download',
            iconName: 'download',
