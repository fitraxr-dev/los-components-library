...
const TabListData = () => {
  ...
  useEffect(() => {
    recordActivity({
      ...
      remarks: 'view report log data interface success',
    });
  }, []);
  ...
  const tanggalKirim = watch('tanggalKirim');
          ...
          <Grid container spacing={isMobile ? 1.5 : 2}>
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <MultipleAutoComplete
                    id="input-typeOfData"
                    label="Type Of Data"
                    placeholder="Choose Type Of Document(s)"
                    dropdownList={typeOfDataOptions}
                    value={field.value || []}
                    onChange={(newValues) => {
                      field.onChange(newValues);
                    }}
            ...
            <Grid item xs={12} md={6}>
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <MultipleAutoComplete
                    id="input-statusDataSent"
                    label="Status Pengiriman Data"
                    placeholder="Choose Type Of Document(s)"
                    dropdownList={statusDataSentOptions}
                    value={field.value || []}
                    onChange={(newValues) => {
                      field.onChange(newValues);
                    }}
