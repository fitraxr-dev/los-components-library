...
import useDownloadReportLogAuditTrailActivity from '@/hooks/services/report/log-audit-trail-activity/useDownloadReportLogAuditTrailActivity';
import useGetHistoryLogAuditTrailActivity from '@/hooks/services/report/log-audit-trail-activity/useGetHistoryLogAuditTrailActivity';
...
const useTabHistoryDownload = () => {
  ...
  // user access
  const canDownloadFile = useCheckAccess(accessid.REPORT_LOG_AUDIT_TRAIL_ACTIVITY_DOWNLOAD);

  // Get report history data
  const { data, isLoading, refetch } = useGetHistoryLogAuditTrailActivity({
    page: {
  ...
  const { mutate: downloadReport, isPending: isDownloading } = useDownloadReportLogAuditTrailActivity({
    onError: (error) => {
    ...
    onSuccess: (response, id) => {
      ...
      recordActivity({
        ...
        remarks: 'download file report log audit trail activity',
      });
