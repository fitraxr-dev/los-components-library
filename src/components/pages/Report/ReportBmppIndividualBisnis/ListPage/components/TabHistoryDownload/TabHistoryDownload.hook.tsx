...
import useDownloadReportBmppIndividualBisnis from '@/hooks/services/report/report-bmpp-individual-bisnis/useDownloadReportBmppIndividualBisnis';
...
const useTabHistoryDownload = () => {
  ...
  const { data, isLoading, refetch } = useGetHistoryBmppIndividualBisnis({
    page: {
  ...
  const { mutate: downloadReport, isPending: isDownloading } = useDownloadReportBmppIndividualBisnis({
    onError: (error) => {
    ...
    onSuccess: (response, id) => {
      ...
      try {
        ...
        recordActivity({
          ...
          remarks: 'download file report bmpp individual bisnis',
        });
      } catch (error) {
  ...
  const getActionOptions = (row: any) => {
    ...
    switch (status) {
      case 'success':
        return [
          {
            apiDownload: 'report.bmppIndividualBisnis.download',
            iconName: 'download',
