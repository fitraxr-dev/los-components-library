...
import {
  addFinalDocumentInitialValues,
  addFinalDocumentModalSchema,
  PDF_MIME,
} from './ModalAddFinalDocument.constants';
import { useModalAddFinalDocument } from './ModalAddFinalDocument.hook';


const ModalAddFinalDocument = NiceModal.create(() => {
  ...
  const modalId = MODAL.RISALAH_RAPAT.ADD_FINAL_DOCUMENT;
  const { visible } = useModal(modalId);

  const { handleOnSave, saveDocumentIsLoading } = useModalAddFinalDocument();
  ...
  const {
    ...
    mode: 'onChange',
    resolver: yupResolver(addFinalDocumentModalSchema),
  });
  ...
  React.useEffect(() => {
    ...
    if (curr.uploadBy !== nextUploadBy || curr.uploadDate !== nextUploadDate) {
      reset(
        { ...curr, uploadBy: nextUploadBy, uploadDate: nextUploadDate },
        { keepDirty: true, keepTouched: true }
      );
    }
  }, [visible, username, today, getValues]);
  ...
  return (
    <SectionModal
      title="Add New Risalah Rapat Final"
    ...
    >
          ...
          <Controller
            ...
            render={({ field, fieldState }) => (
              <Input
                ...
                disabled={saveDocumentIsLoading}
                isMandatory
          ...
          <Controller
            ...
            render={({ field, fieldState }) => (
              <Input
                ...
                error={fieldState.isTouched && fieldState.invalid}
                helperText={fieldState.isTouched && fieldState.error ? fieldState.error.message : ''}
                disabled={!document?.name || saveDocumentIsLoading}
                isMandatory
              />
            )}
          />
            ...
            <Controller
              ...
              render={({ field, fieldState }) => (
                <Input
                  ...
                  regex={/^[^\s][\w\d.,()_\s\-@\/]*$/}
                  helperText={fieldState.isTouched && fieldState.error ? fieldState.error.message : ''}
                  disabled={!documentName || saveDocumentIsLoading}
                />
              )}
            />
      ...
      <RowWrapper sx={{ gap: 2, justifyContent: 'end', mt: 3 }}>
        ...
        <Button
          isLoading={saveDocumentIsLoading}
...
export default ModalAddFinalDocument;
