...
const useGetRequestVaList = (payload: GenericBucketRequestDtoVAListFilterDto) => {
  const query = useQuery({
    placeholderData: keepPreviousData,
    queryFn: async () => {
      const res = await api.getRequestVAList(payload);

      return res.data.data;
    },
    queryKey: ['va-list-request', payload],
  });
  return query;
};

export default useGetRequestVaList;
