...
const useGetCurrentVaList = (payload: GenericBucketRequestDtoVAListFilterDto) => {
  const query = useQuery({
    placeholderData: keepPreviousData,
    queryFn: async () => {
      const res = await api.getCurrentVAList(payload);

      return res.data.data;
    },
    queryKey: ['va-list-current', payload],
  });
  return query;
};

export default useGetCurrentVaList;
