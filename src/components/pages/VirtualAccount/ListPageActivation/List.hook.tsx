...
import useGetActivation from '../hooks/useGetActivation';
...
const useList = () => {
  ...
  const { isStaff, isSuperAdmin, isStaffDkhi, isTL, isMaker, isChecker } = useVirtualAccountContext();
  const canViewVa = useCheckAccess(accessid.VIRTUAL_ACCOUNT_ACTIVATION_VIEW);
  ...
  const { data: statusActivationList } = useGetParameterList('filterStatusActivationVA');
  const statusActivationOptions = statusActivationList?.map((item: any) => ({
    label: item.label,
    value: item.label,
  })) || [];
  const searchOptions = [
  ...
  const statusByOptions =
    statusData
      ?.filter(
        (item) => item.label === 'Waiting VA Activation' || item.label === 'Completed'
      )
      .map((item) => ({
        label: item.label,
        value: item.key,
  ...
  const { data: vaList, isFetching: isLoading } = useGetActivation({
    filter: {
      ...filter?.filter,
  ...
  useEffect(() => {
    if (vaList) {
      recordActivity({
        ...
        module: TypeModule.VA_CREATION,
        process: TypeProcess.VA_CREATION,
        remarks: 'view virtual account list activation',
      });
    }
  ...
  const tableHeader: TableHeader[] = [
    ...
    ...(isStaffDkhi || isMaker || isChecker ? [{
      key: 'pic',
      label: 'PIC',
      sx: { minWidth: '10vw' },
    }] : []),
    ...(isStaffDkhi || isMaker || isChecker ? [{
      key: 'statusActivation',
      label: 'Status Activation',
      sx: { minWidth: '8vw' },
    }] : []),
    {
      ...
      options: [
        ...(canViewVa ? [{
          ...
          onClick: (data) => {
            recordActivity({
              activity: ActivityType.VIEW,
              bucketProcessId: data.bucketProcessId || 'VA-ID',
              module: TypeModule.VA_CREATION,
              process: TypeProcess.VA_CREATION,
              remarks: `view detail activation for customer ${data.customerName}`,
            });
            const nextPath = replacePath(virtualAccount.VA_ACTIVATION_DETAIL_CUSTOMER,
              { processId: `${data?.customerId}~${data?.bucketProcessId ?? 'VA-ID'}~VL~AC` });
  ...
  const filterDropdownList = isStaffDkhi || isMaker || isChecker ? searchOptions : searchByOptions;

  const filterContentList = [
    {
      key: 'sortList',
      label: 'Urutkan Berdasarkan',
      options: isStaffDkhi || isMaker || isChecker ? sortOptions ?? [] : sortByOptions ?? [],
      type: 'sort',
    },
    {
      endKey: 'endDate',
    ...
    },
    {
      key: 'statusActivation',
      label: 'Status Activation',
      options: statusActivationOptions,
      type: 'multiple-autocomplete',
    },
  ];
