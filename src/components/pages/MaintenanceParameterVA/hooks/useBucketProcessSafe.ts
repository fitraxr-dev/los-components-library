...
export const useBucketProcessSafe = (): UseBucketProcessSafeReturn => {
  ...
  React.useEffect(() => {
    if (typeof window !== 'undefined') {
      const sessionData = sessionStorage.getItem('maintenanceParameterVANavigation');
      if (sessionData) {
        try {
      ...
      if (isSuccess && registerResponse.data) {
        ...
        if (newBucketProcessId) {
          ...
          const navigationData = {
            ...
            isEdit: true,
            isViewOnly: false,
            key: registerResponse.data.bankName || '',
            registerData: registerResponse.data,
          };

          sessionStorage.setItem('maintenanceParameterVANavigation', JSON.stringify(navigationData));
