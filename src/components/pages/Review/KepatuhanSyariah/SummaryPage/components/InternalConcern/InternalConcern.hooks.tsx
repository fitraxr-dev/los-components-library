...
const useInternConcern = () => {
  ...
  const { data } = useGetSummaries({
    ...
    type: TypeSummary.INTERNAL,
  });


  const { mutate } = useDeleteConcern(
    {
      onError: () => {
  ...
  const handleEdit = (id: number, idx: number) => {
    const idQueue = idx + 1;
    router.push(replacePath(KEPATUHAN_SYARIAH.EDIT_SUMMARY + '?' + createQueryString('type', 'internal', String(idQueue)), {
      id: id,
      module: moduleIndex,
      processId: processId,
    }));
  ...
  const handleNewData = () => {
    if (!viewOnly) {
      ...
      router.push(replacePath(KEPATUHAN_SYARIAH.ADD_SUMMARY.replace('[module]', moduleIndex) + '?' + createQueryString('type', 'internal', String(idQueue)), {
        processId: processId,
      }));
    }
