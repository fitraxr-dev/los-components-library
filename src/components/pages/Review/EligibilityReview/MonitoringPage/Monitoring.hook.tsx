...
export const useMonitoring = () => {
  ...
  const [filter, setFilter] = useSessionStorage('filter-component-eligibilityreview-monitoring', null);
  ...
  const canViewEligibility = useCheckAccess(accessid.MONITORING_ELIGIBILITY_REVIEW_VIEW);

  const { data: filterStatusByOptions } = useGetParameterList('filterStatusMIPReviewDEPIList');
  const { data: searchByOptions } = useGetParameterList('searchByMIPReviewDEPIList', {
    label: 'value1',
    value: 'value2',
  });
  const { data: sortByOptions } = useGetParameterList('sortByMIPReviewDEPIList', {
    label: 'value1',
    value: 'value2',
  });
