...
export const NavigationProvider: React.FC<NavigationProviderProps> = ({
  ...
  const setNavigationData = useCallback((newData: Partial<NavigationData>) => {
    setNavigationDataState((prevData) => {
      ...
      if (typeof window !== 'undefined') {
        try {
          sessionStorage.setItem('maintenanceParameterNavigation', JSON.stringify(updatedData));
        } catch (error) {
  ...
  const navigateToEdit = useCallback((data: any) => {
    setNavigationData({
      ...
      bucketProcessId: data.bucketProcessId || null,
      id: data.id || null,
      isBucketListDetail: false, // Always false for edit mode to enable Summary and Validation tabs
      isDetail: true,
      isEdit: true,
      isViewOnly: false,
      key: data.id || '',
