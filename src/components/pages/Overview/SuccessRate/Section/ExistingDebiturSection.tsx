...
interface ExistingDebiturSectionProps {
  data: {
    color: string;
    name: string;
    value: number;
...
const ExistingDebiturSection: React.FC<ExistingDebiturSectionProps> = ({ data }) => {
  ...
  const formatCurrencyShort = (value: number) => {
    if (value >= 1_000_000_000) {
      ...
      return `IDR ${(value / 1_000_000).toFixed(0)} Juta`;
    }
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      ...
      return (
        ...
        >
          <Box sx={{ fontSize: isMobile ? 11 : 12 }}>
            Total Plafond : {data.plafond !== undefined ? formatCurrencyShort(data.plafond) : 'N/A'}
          </Box>
          <Box sx={{ fontSize: isMobile ? 11 : 12 }}>
            Pengurangan : {data.pengurangan !== undefined ? formatCurrencyShort(data.pengurangan) : 'N/A'}
          </Box>
          <Box sx={{ fontSize: isMobile ? 11 : 12 }}>
            Penambahan : {data.penambahan !== undefined ? formatCurrencyShort(data.penambahan) : 'N/A'}
          </Box>
        </Box>
      );
    }
  ...
  const chartHeight = isMobile ? 150 : '100%';
...
export default ExistingDebiturSection;
