...
const useGenerateReportExcelTatSlaSum = (
  ...
  const mutation = useMutation({
    mutationFn: async (payload: any) => {
      try {
        ...
        const response = await API('report.tatSlaSum.excel', { data: { filter: payload } });
...
export default useGenerateReportExcelTatSlaSum;
