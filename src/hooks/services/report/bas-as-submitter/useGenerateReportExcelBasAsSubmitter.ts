...
const useGenerateReportExcelBasAsSubmitter = (
  ...
  const mutation = useMutation({
    mutationFn: async (payload: any) => {
      try {
        ...
        const response = await API('report.basAsSubmitter.excel', { data: { filter: payload } });
...
export default useGenerateReportExcelBasAsSubmitter;
