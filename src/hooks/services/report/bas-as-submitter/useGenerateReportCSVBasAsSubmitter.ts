...
const useGenerateReportCSVBasAsSubmitter = (
  ...
  const mutation = useMutation({
    mutationFn: async (payload: any) => {
      try {
        ...
        const response = await API('report.basAsSubmitter.csv', { data: { filter: payload } });
...
export default useGenerateReportCSVBasAsSubmitter;
