...
const useGenerateReportCSVBmppGroupDpop = (
  ...
  const mutation = useMutation({
    mutationFn: async (payload: any) => {
      try {
        ...
        const response = await API('report.bmppGroupDpop.csv', { data: { filter: payload } });
...
export default useGenerateReportCSVBmppGroupDpop;
