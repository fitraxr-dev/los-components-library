...
const useGenerateReportExcelApuPpt = (
  ...
  const mutation = useMutation({
    mutationFn: async (payload: any) => {
      try {
        ...
        const response = await API('report.assessmentApuPpt.excel', { data: { filter: payload } });
...
export default useGenerateReportExcelApuPpt;
